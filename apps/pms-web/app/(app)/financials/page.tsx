...
function KpiCard({
  ...
  return (
    <div
      className={`flex flex-col justify-center min-w-[180px] px-4 py-3 rounded-xl border bg-white ${
    ...
    >
      ...
      {deltaLabel != null && (
        <p
          className={`flex items-center gap-1 text-[11px] mt-1 ${trendUp ? "text-emerald-600" : "text-rose-600"}`}
        >
          {trendUp ? (
      ...
      {sublabel && <p className="text-[11px] text-amber-700 mt-1 font-medium">{sublabel}</p>}
    </div>
  );
}
