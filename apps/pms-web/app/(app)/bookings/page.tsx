        ...
        {visibleTabs.map((tab) => {
          ...
          return (
            ...
            >
              {tab.label}
              {tab.count > 0 && (
                <span className={`ml-1 text-xs ${isActive ? 'text-gray-500' : 'text-gray-400'}`}>({tab.count})</span>
              )}
              {isActive && (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-gray-400 mt-0.5 font-mono truncate">{b.bookingReference}</p>
                  </div>
                  <span className={`shrink-0 inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold border ${STATUS_STYLES[b.status] || STATUS_STYLES['pending']}`}>
                ...
                <div className="space-y-1 text-[12px] text-gray-600 border-t border-gray-100 pt-2.5">
                  <div className="flex items-center gap-1.5 truncate">
                    ...
                    {b.roomNumber ? (
                      <span className="truncate"><span className="font-medium text-gray-800">#{b.roomNumber}</span> · {b.roomName}</span>
                    ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-center px-4 pb-3 pt-4 text-xs font-medium text-gray-400 w-[90px]">{t('bookings.tableBalance')}</th>
                <th className="text-left px-4 pb-3 pt-4 text-xs font-medium text-gray-400 w-[70px]">{t('bookings.tableSource')}</th>
                  ...
                  <tr key={b.id} className="border-b border-gray-100 last:border-b-0 hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-4 py-4">
                      <Link href={`/bookings/${b.id}`} className="block">
                        ...
                        <p className="text-[11px] text-gray-400 mt-0.5 font-mono">{b.bookingReference}</p>
                      </Link>
                    </td>

                    {/* Status */}
                    <td className="px-4 py-4">
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-semibold border ${STATUS_STYLES[b.status] || STATUS_STYLES['pending']}`}>
                        {STATUS_LABEL_KEYS[b.status] ? t(STATUS_LABEL_KEYS[b.status]) : b.status}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-4 text-center">
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-semibold border ${BALANCE_STYLES[balance] || BALANCE_STYLES['due']}`}>
                        {balance === 'paid' ? t('bookings.balancePaid') : balance === 'partial' ? t('bookings.balancePartial') : balance === 'refunded' ? t('bookings.balanceRefunded') : t('bookings.balanceDue')}
                      </span>
                    </td>
