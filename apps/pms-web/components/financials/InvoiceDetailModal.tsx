            ...
            <>
              ...
              <div className="sticky top-0 z-10 bg-white px-6 pt-6 pb-4 flex items-start justify-between gap-4 border-b border-gray-100 print:static print:border-b-0">
                <div>
                  <div className="flex items-center gap-3">
                    <h2 className="text-lg font-bold text-gray-900 font-mono">{invoice.invoiceNumber}</h2>
                    <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-semibold border ${STATUS_STYLES[invoice.status]}`}>
                      {t(`financials.status${invoice.status.charAt(0).toUpperCase() + invoice.status.slice(1)}`)}
                    </span>
                  </div>
                </div>
                <button
              ...
              <div className="px-6 pb-6 space-y-5">
                ...
                <div className="text-sm space-y-1.5">
                  ...
                  <Row icon={<EnvelopeIcon className="w-4 h-4 text-gray-400" />}>{invoice.guestEmail}</Row>
                  <Row icon={<HomeIcon className="w-4 h-4 text-gray-400" />}>
                    {invoice.roomNumber ? `Room ${invoice.roomNumber} · ` : ''}{invoice.roomName}
                  </Row>
                  <Row icon={<CalendarIcon className="w-4 h-4 text-gray-400" />}>
                    {formatDate(invoice.checkIn)} <span className="text-gray-300">→</span> {formatDate(invoice.checkOut)}
                  </Row>
                </div>
                ...
                <div className="space-y-1.5 pt-3 border-t border-gray-100 text-sm">
                  ...
                  <div className="flex justify-between font-bold text-gray-900 text-base pt-1">
                    ...
                    <span className="tabular-nums">{formatCurrency(invoice.totalAmount, invoice.currency)}</span>
                  </div>
                </div>
                ...
                <div className="space-y-1.5 pt-3 border-t border-gray-100 text-sm">
                  ...
                  <div className="flex justify-between font-bold">
                    ...
                    <span className={`tabular-nums ${invoice.balanceDue > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                ...
                <div className="space-y-2 pt-2 print:hidden">
                  {invoice.balanceDue > 0 && invoice.status !== 'voided' && (
                    ...
                    >
                      ...
                      {t('financials.actionRecordPayment', { amount: formatCurrency(invoice.balanceDue, invoice.currency) })}
                    </button>
                  )}
                  <div className="grid grid-cols-3 gap-2">
                    <ActionButton icon={<PaperAirplaneIcon className="w-4 h-4" />} label={t('financials.actionResend')} disabled />
                    <ActionButton icon={<ArrowDownTrayIcon className="w-4 h-4" />} label={t('financials.actionPdf')} disabled />
                    <ActionButton
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <ActionButton icon={<ReceiptRefundIcon className="w-4 h-4" />} label={t('financials.actionCreditNote')} disabled />
                    <ActionButton
