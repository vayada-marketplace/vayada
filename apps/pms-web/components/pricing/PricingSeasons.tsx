...
export function changeSeasonPrice(room: PricingConfiguration, offerId: string, season: SeasonInput, values: string[] | null): PricingConfiguration {
...
export function PricingSeasons({ room, offer, label, disabled, onChange, onPending }: { room: PricingConfiguration; offer: Offer; label: string; disabled: boolean;
  ...
  const [entry, setEntry] = useState<SeasonInput>({ name: "", tier: "", from: "", through: "" }), [values, setValues] = useState<string[]>([]), [error, setError] = useState("");
  if (offer.price.kind !== "independent") return null;
  const template = recurringTemplate(offer), scale = pricingCurrencyScale(room.currency)!;
  const pending = Object.values(entry).some(Boolean) || values.some(Boolean);
  const reset = () => { setEntry({ name: "", tier: "", from: "", through: "" }); setValues([]); setError(""); onPending(false); };
  const apply = (season: SeasonInput, amounts: string[] | null) => {
    if (disabled) return;
    try { onChange(changeSeasonPrice(room, offer.id, season, amounts)); if (amounts) reset(); else setError(""); }
  ...
  return <details className="sm:col-span-2 text-sm"><summary className="cursor-pointer">Seasonal prices · {label}</summary>
    ...
    <ul className="my-3 space-y-2">{offer.price.calendar.seasons.map((season) => <li key={season.from} className="flex flex-wrap items-center gap-3">
      ...
      <button type="button" className="rounded border px-3 py-1 disabled:opacity-50" disabled={disabled} aria-label={`Clear season ${season.from} to ${season.through} for ${label}`} onClick={() => apply(season, null)}>Clear seasonal price</button>
    </li>)}</ul>
    {!template ? <p>Set up recurring pricing before adding seasonal prices. This offer has no recurring price template.</p> : <>
      <p className="mb-3">Seasonal prices use this offer’s current pricing mode.{recurringAdjustments(template, room.currency, scale)}{template.mode === "included_guests" && " These included-adult settings stay the same when adding a season."}</p>
      <div className="flex flex-wrap items-end gap-3">
        ...
        <button type="button" className="rounded border px-3 py-2 disabled:opacity-50" disabled={disabled} onClick={() => apply(entry, values)}>Add seasonal price</button>
      </div>
    </>}
    {pending && <><button type="button" className="mt-3 rounded border px-3 py-2 disabled:opacity-50" disabled={disabled} onClick={reset}>Cancel season entry</button><p className="mt-2">Add or cancel this season entry before saving the draft.</p></>}
