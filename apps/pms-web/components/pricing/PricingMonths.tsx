...
export function changeMonthPrice(room: PricingConfiguration, offerId: string, month: string, values: string[] | null, replace = false): PricingConfiguration {
  ...
  if (values) {
    const price = recurringPrice(offer, values, room.currency, replace ? existing!.price : recurringTemplate(offer));
...
export function PricingMonths({ room, offer, label, disabled, onChange, onPending }: { room: PricingConfiguration; offer: Offer; label: string; disabled: boolean;
  ...
  const reset = () => { setMonth(""); setValues([]); setEditing(false); setError(""); onPending(false); };
  const apply = (selected: string, amounts: string[] | null) => {
    if (disabled || (!amounts && pending)) return;
    try { onChange(changeMonthPrice(room, offer.id, selected, amounts, editing)); if (amounts) reset(); else setError(""); }
  ...
  return <details className="sm:col-span-2 text-sm"><summary className="cursor-pointer">Monthly prices · {label}</summary>
    ...
    <ul className="my-3 space-y-2">{offer.price.calendar.months.map((entry) => <li key={entry.month} className="flex flex-wrap items-center gap-3">
      ...
      <button type="button" className="rounded border px-3 py-1 disabled:opacity-50" disabled={disabled || pending} aria-label={`Edit ${months[entry.month - 1]} price for ${label}`} onClick={() => {
        ...
        setEditing(true); setMonth(String(entry.month)); setValues(baseAmounts(entry.price).map(([, minor]) => decimalAmount(minor, scale))); setError(""); onPending(true);
    ...
    {!template ? <p>Set up recurring pricing before adding monthly prices. This offer currently has only date prices or no recurring price.</p> : <>
      <p className="mb-3">{editing ? "Edit this month’s prices while keeping its pricing mode and included-adult settings. Other months and rules remain unchanged." : "Monthly prices use this offer’s current pricing mode."}{recurringAdjustments(template, room.currency, scale)}{!editing && template.mode === "included_guests" && " These included-adult settings stay the same when adding a month."}</p>
