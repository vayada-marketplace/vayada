...
export function changeDatePrice(room: PricingConfiguration, offerId: string, date: string, amount: string | string[] | null, useRecurring = false): PricingConfiguration {
  ...
  const nextDates = useRecurring && Array.isArray(amount) ? [...dates, { date, price: recurringPrice(offer, amount, room.currency) }] : Array.isArray(amount) ? dates.map((entry) => entry.date === date ? { ...entry, price: recurringPrice(offer, amount, room.currency, existing!.price) } : entry) : amount === null ? dates.filter((entry) => entry.date !== date) : [...dates, { date, price: { mode: "flat" as const, amountMinor: parseMinorInput(amount, pricingCurrencyScale(room.currency)!) } }];
...
export function PricingDates({ room, offer, label, disabled, onChange, onPending }: { room: PricingConfiguration; offer: Offer; label: string; disabled: boolean;
  ...
  const reset = () => { setDate(""); setAmounts([]); setEditing(false); setUseRecurring(false); setError(""); onPending(false); };
  const apply = (day: string, value: string | string[] | null) => {
    if (disabled || (value === null && pending)) return;
    try { onChange(changeDatePrice(room, offer.id, day, value, !editing && useRecurring)); if (value !== null) reset(); else setError(""); }
