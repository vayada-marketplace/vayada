...
export function changeDatePrice(room: PricingConfiguration, offerId: string, date: string, amount: string | string[] | null, useRecurring = false, included: IncludedInput | null = null): PricingConfiguration {
  ...
  if (included && ((editing ? existing?.price : useRecurring ? recurringTemplate(offer) : null)?.mode !== "included_guests" || !Array.isArray(amount) || amount.length !== 1)) throw new Error("Choose included-adult date pricing.");
  const nextDates = useRecurring && Array.isArray(amount) ? [...dates, { date, price: included ? includedPrice(included, amount[0], room.capacity.adults, pricingCurrencyScale(room.currency)!) : recurringPrice(offer, amount, room.currency) }] : Array.isArray(amount) ? dates.map((entry) => entry.date === date ? { ...entry, price: included ? includedPrice(included, amount[0], room.capacity.adults, pricingCurrencyScale(room.currency)!) : recurringPrice(offer, amount, room.currency, existing!.price) } : entry) : amount === null ? dates.filter((entry) => entry.date !== date) : [...dates, { date, price: { mode: "flat" as const, amountMinor: parseMinorInput(amount, pricingCurrencyScale(room.currency)!) } }];
...
export function PricingDates({ room, offer, label, disabled, onChange, onPending }: { room: PricingConfiguration; offer: Offer; label: string; disabled: boolean;
  ...
  const fields = template ? baseAmounts(activeIncluded && template.mode === "included_guests" ? { ...template, baseGuests: Number(activeIncluded.adults) || template.baseGuests } : template) : [["Per room", ""]];
  return <details className="sm:col-span-2 text-sm"><summary className="cursor-pointer">Date-specific prices · {label}</summary>
    ...
    {!editing && <>
      {recurring && <label className="mb-3 block">New date pricing<select aria-label={`New date pricing for ${label}`} className="mt-1 block rounded border px-3 py-2" disabled={disabled} value={useRecurring ? "recurring" : "flat"} onChange={(event) => {
        if (disabled) return; const next = event.target.value === "recurring"; setUseRecurring(next); setAmounts([]); setIncluded(null); setError(""); onPending(next || !!date);
    ...
    <div className="flex flex-wrap items-end gap-3">
      ...
      {activeIncluded && <div className="w-full"><p>The base amount above belongs only to this date price. Changing the included count clears its adjustments.</p><IncludedPricing value={activeIncluded} label={`${label} date price`} capacity={room.capacity.adults} disabled={disabled} onChange={(next) => { if (disabled) return; setIncluded(next); setError(""); onPending(true); }} /></div>}
