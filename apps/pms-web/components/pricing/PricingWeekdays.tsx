...
export function changeWeekdayPrice(room: PricingConfiguration, offerId: string, day: string, input: { kind: string; value: string } | null): PricingConfiguration {
  ...
  if (input && exists) throw new Error("Clear the existing weekday adjustment before adding a replacement.");
...
export function PricingWeekdays({ room, offer, label, disabled, onChange, onPending }: { room: PricingConfiguration; offer: Offer; label: string; disabled: boolean;
  ...
  const [entry, setEntry] = useState({ day: "", kind: "", value: "" }), [error, setError] = useState("");
  ...
  const reset = () => update({ day: "", kind: "", value: "" });
  const apply = (day: string, input: { kind: string; value: string } | null) => {
    if (disabled) return;
    try { onChange(changeWeekdayPrice(room, offer.id, day, input)); if (input) reset(); else setError(""); }
  ...
  return <details className="sm:col-span-2 text-sm"><summary className="cursor-pointer">Weekday adjustments · {label}</summary>
    ...
    <ul className="my-3 space-y-2">{offer.price.calendar.weekdays.map(({ day, adjustment }) => <li key={day} className="flex flex-wrap items-center gap-3">
      ...
      <button type="button" className="rounded border px-3 py-1 disabled:opacity-50" disabled={disabled} aria-label={`Clear ${weekdayNames[day]} adjustment for ${label}`} onClick={() => apply(String(day), null)}>Clear weekday adjustment</button>
    </li>)}</ul>
    <div className="flex flex-wrap items-end gap-3">
      <label>Weekday<select aria-label={`Weekday for ${label}`} className="mt-1 block rounded border px-3 py-2" disabled={disabled} value={entry.day} onChange={(event) => update({ ...entry, day: event.target.value })}>
      ...
      <button type="button" className="rounded border px-3 py-2 disabled:opacity-50" disabled={disabled} onClick={() => apply(entry.day, entry)}>Add weekday adjustment</button>
    ...
    {pending && <p className="mt-2">Add or cancel this weekday entry before saving the draft.</p>}
