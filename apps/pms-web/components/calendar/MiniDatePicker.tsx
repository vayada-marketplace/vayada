  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="text-sm font-semibold text-gray-900">{format(viewMonth, "MMMM yyyy")}</div>
        <button
          type="button"
          onClick={goNext}
      ...
      <div className="grid grid-cols-7 gap-y-0.5">
        {weeks.flat().map((d, i) => {
          ...
          let cls =
            "flex items-center justify-center w-9 h-9 text-[13px] rounded-full transition-colors";
          if (!inMonth) {
          ...
          return (
            <button key={i} type="button" onClick={() => pick(d)} className={cls}>
              {format(d, "d")}
            </button>
          );
        })}
