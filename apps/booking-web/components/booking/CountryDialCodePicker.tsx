...
export default function CountryDialCodePicker({ value, onChange }: CountryDialCodePickerProps) {
  ...
  const filtered = useMemo(() => COUNTRY_DIAL_CODES.filter((c) => matches(c, query)), [query]);

  const selected = useMemo(() => COUNTRY_DIAL_CODES.find((c) => c.iso2 === value) ?? null, [value]);
  ...
  const buttonLabel = selected ? `${selected.flag} +${selected.dial}` : t("selectCountryCode");
      ...
      {open && (
        <div className="absolute z-20 left-0 top-full mt-1 w-72 max-w-[calc(100vw-2rem)] rounded-lg border border-gray-300 bg-white shadow-lg">
          ...
          <ul ref={listRef} id={listboxId} role="listbox" className="max-h-64 overflow-y-auto py-1">
            {filtered.length === 0 ? (
              <li className="px-3 py-2 text-sm text-gray-500">{t("noCountriesFound")}</li>
            ) : (
              filtered.map((c, i) => {
