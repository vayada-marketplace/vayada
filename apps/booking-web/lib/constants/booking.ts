...
export function getNonRefundableNightlyRates(room: RoomType | undefined, nights: number): number[] {
  if (!room) return [];
  if (room.nonRefundableNightlyRates && room.nonRefundableNightlyRates.length === nights) {
    return room.nonRefundableNightlyRates;
  }
  const flexibleRates = getFlexibleNightlyRates(room, nights);
  if (room.nonRefundableRate != null) {
    return normalizeNightlyRates(room.nonRefundableNightlyRates, room.nonRefundableRate, nights);
  }
  return flexibleRates.map((rate) => getNonRefundableRate(rate, null));
}

export function hasVariableNightlyRates(rates: number[]): boolean {
  return new Set(rates.map((rate) => rate.toFixed(2))).size > 1;
}
...
export function calculateAddonTotal(
  ...
  for (const addon of addons) {
    ...
    const qty = addon.perNight ? (quantities?.[addon.id] ?? nights) : (quantities?.[addon.id] ?? 1);
