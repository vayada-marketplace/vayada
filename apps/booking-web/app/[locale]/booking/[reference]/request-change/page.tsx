...
export default function RequestChangePage({
  ...
  useEffect(() => {
    if (!guestEmail) {
      setLoadError(t('missingEmail') || 'This page must be opened from the booking confirmation link.')
      setLoading(false)
      return
    }
    let cancelled = false
  ...
  useEffect(() => {
    ...
    booking, hasPending, slug, guestEmail,
    checkIn, checkOut,
    selectedAddonIds, addonQuantities, addonDates,
  ])

  const toggleAddon = useCallback((addonId: string) => {
    setSelectedAddonIds(prev => prev.includes(addonId)
  ...
  const noChange = useMemo(() => {
    ...
    return checkIn === booking.checkIn && checkOut === booking.checkOut && selectedAddonIds.length === 0
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-1 w-full">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">{t('title') || 'Request Booking Changes'}</h1>
        <p className="text-sm text-gray-600 mb-6">
          {t('subtitle') || "Pick the new dates or add-ons you'd like. The hotel will review and approve before anything is applied."}
        </p>
        ...
        {hasPending && (
          <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
            {t('pendingExists') || 'A change request is already pending approval for this booking. Please wait for the hotel to respond.'}
          </div>
        )}
        ...
        {booking && !hasPending && (
          <>
            <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-4">
              <h2 className="text-sm font-semibold text-gray-900 mb-3">{t('newDates') || 'New dates'}</h2>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">{t('checkIn') || 'Check-in'}</label>
                  <input
                    type="date"
                    value={checkIn}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">{t('checkOut') || 'Check-out'}</label>
                  <input
                    type="date"
                    value={checkOut}
            ...
            {addons.length > 0 && (
              <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-4">
                <h2 className="text-sm font-semibold text-gray-900 mb-3">{t('addons') || 'Add-ons'}</h2>
                <p className="text-xs text-gray-500 mb-3">
                  {t('addonsHint') || 'Picking nothing here keeps the booking without add-ons; selecting an add-on adds it to the booking once the hotel approves.'}
                </p>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-4">
              <h2 className="text-sm font-semibold text-gray-900 mb-3">{t('summary') || 'Summary'}</h2>
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between pt-2 border-t border-gray-100">
                  <span className="font-semibold text-gray-900">{t('priceDifference') || 'Price difference'}</span>
                  <span className={`font-bold ${preview && preview.priceDifference > 0 ? 'text-amber-600' : preview && preview.priceDifference < 0 ? 'text-green-600' : 'text-gray-900'}`}>
                    {preview ? (
            ...
            >
              {submitting ? (t('submitting') || 'Submitting…') : (t('submit') || 'Submit Change Request')}
            </button>
          </>
        )}
      </div>
