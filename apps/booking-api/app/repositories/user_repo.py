...
class UserRepository:
    @staticmethod
    async def get_by_email(email: str, *, columns: str = "*") -> dict | None:
        row = await AuthDatabase.fetchrow(f"SELECT {columns} FROM users WHERE email = $1", email)
        return dict(row) if row else None

    @staticmethod
    async def get_by_id(user_id: str, *, columns: str = "*") -> dict | None:
    ...
    async def create(
        ...
        row = await AuthDatabase.fetchrow(
            query, email, password_hash, name, terms_version, privacy_version, marketing_consent
        )
        return dict(row)
    ...
    @staticmethod
    async def update_email(user_id: str, new_email: str) -> dict | None:
        row = await AuthDatabase.fetchrow(
