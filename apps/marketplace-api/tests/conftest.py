...
async def test_hotel_user(db_setup):
    """Create a test hotel user and return user_id"""
    from app.database import Database
    import bcrypt
    import uuid
    ...
    # Clean up if exists
    await Database.execute(
        "DELETE FROM hotel_listings WHERE hotel_profile_id IN (SELECT id FROM hotel_profiles WHERE user_id IN (SELECT id FROM users WHERE email = $1))",
        test_email
    )
    await Database.execute(
        "DELETE FROM hotel_profiles WHERE user_id IN (SELECT id FROM users WHERE email = $1)",
        test_email
    )
    await Database.execute(
        "DELETE FROM users WHERE email = $1",
    ...
    # Cleanup
    await Database.execute(
        "DELETE FROM hotel_listings WHERE hotel_profile_id IN (SELECT id FROM hotel_profiles WHERE user_id = $1)",
        user['id']
    )
    await Database.execute(
...
@pytest.fixture
async def test_hotel_profile(test_hotel_user, db_setup):
    """Create a test hotel profile"""
    from app.database import Database
    ...
    yield str(hotel_profile['id']), test_hotel_user
    
    # Cleanup is handled by test_hotel_user fixture


@pytest.fixture
def hotel_auth_headers(test_hotel_user):
    """Get authentication headers with JWT token for test_hotel_user"""
    token = get_token_for_user(test_hotel_user)
    if not token:
        return {}
    return {"Authorization": f"Bearer {token}"}
