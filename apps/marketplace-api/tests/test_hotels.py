...
class TestHotelProfileStatus:
    ...
    def test_get_profile_status_incomplete_profile_with_defaults(self, client, test_hotel_user):
        ...
        assert data["has_defaults"]["category"] is True
        ...
        assert "Set a custom location" in data["completion_steps"] or "Update your hotel category" in data["completion_steps"]
    ...
    async def test_get_profile_status_complete_profile(self, client, db_setup):
        ...
        hotel = await Database.fetchrow(
            """
            INSERT INTO hotel_profiles (user_id, name, category, location, email, website, about)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            RETURNING id
            """,
            user['id'],
            "Grand Hotel",
            "Luxury Hotel",  # Not default
        ...
        try:
            ...
            assert data["has_defaults"]["category"] is False
    ...
    async def test_get_profile_status_default_location(self, client, db_setup):
        ...
        await Database.execute(
            """
            INSERT INTO hotel_profiles (user_id, name, category, location, email, about)
            VALUES ($1, $2, $3, $4, $5, $6)
            """,
            user['id'],
            "Test Hotel",
    ...
    @pytest.mark.asyncio
    async def test_get_profile_status_default_category(self, client, db_setup):
        """Test profile status when category has default value"""
        import bcrypt
        import uuid
        
        test_email = f"defaultcat_hotel_{uuid.uuid4().hex[:8]}@test.com"
        password_hash = bcrypt.hashpw("testpassword123".encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
        ...
        # Create hotel profile with default category
        await Database.execute(
            """
            INSERT INTO hotel_profiles (user_id, name, category, location, email, about)
            VALUES ($1, $2, $3, $4, $5, $6)
        ...
        try:
            ...
            assert data["has_defaults"]["location"] is False
            assert data["has_defaults"]["category"] is True
            assert "Update your hotel category" in data["completion_steps"]
        finally:
            # Cleanup
            await Database.execute("DELETE FROM users WHERE id = $1", user['id'])
    ...
    async def test_get_profile_status_missing_fields(self, client, db_setup):
        ...
        await Database.execute(
            """
            INSERT INTO hotel_profiles (user_id, name, category, location, email, about, website)
