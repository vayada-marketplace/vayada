...
@router.post(
    "/export-request", response_model=GdprExportRequestResponse, status_code=status.HTTP_201_CREATED
)
async def request_data_export(
    request: Request, user_id: str = Depends(get_current_user_id_allow_pending)
):
    """
    ...
    try:
        ...
        if creator:
            ...
            user_data["creator_platforms"] = [
                {k: str(v) if v else None for k, v in p.items()} for p in platforms
            ]
...
async def download_data_export(
    token: str, user_id: str = Depends(get_current_user_id_allow_pending)
):
    """
    ...
    try:
        ...
        if not request_record:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Invalid or expired download token"
            )
        ...
        # Check if expired
        if request_record["expires_at"] < datetime.now(UTC):
            raise HTTPException(
...
async def _collect_user_data(user_id: str) -> dict:
    ...
    user_data = {"export_date": datetime.now(UTC).isoformat()}
...
@router.get(
    "/export-status", response_model=GdprRequestStatusResponse, status_code=status.HTTP_200_OK
)
async def get_export_status(user_id: str = Depends(get_current_user_id_allow_pending)):
    ...
    try:
        ...
        if not result:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No export request found"
            )
...
@router.post(
    "/delete-request",
    response_model=GdprDeletionRequestResponse,
...
async def request_account_deletion(
    ...
    try:
        ...
        scheduled_deletion = datetime.now(UTC) + timedelta(days=DELETION_GRACE_PERIOD_DAYS)
        ...
        logger.info(
            f"Account deletion requested for user {user_id}, scheduled for {scheduled_deletion}"
        )
...
@router.post(
    "/delete-cancel", response_model=GdprDeletionCancelResponse, status_code=status.HTTP_200_OK
)
async def cancel_account_deletion(
    ...
    try:
        ...
        if not existing:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No pending deletion request found"
            )
...
@router.get(
    "/delete-status", response_model=GdprRequestStatusResponse, status_code=status.HTTP_200_OK
)
async def get_deletion_status(user_id: str = Depends(get_current_user_id_allow_pending)):
    ...
    try:
        ...
        if not result:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No deletion request found"
            )
