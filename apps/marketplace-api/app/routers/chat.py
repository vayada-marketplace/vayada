...
async def get_chat_messages(
    ...
    return [
        ChatMessageResponse(
            ...
            read_at=m["read_at"],
            sender_name=sender_names_map.get(str(m["sender_id"]), "System")
            if m["sender_id"]
            else "System",
...
async def send_chat_message(
    ...
    row = await ChatRepository.send_message(
        collaboration_id, user_id, request.content, request.message_type
    )
...
async def mark_messages_as_read(collaboration_id: str, user_id: str = Depends(get_current_user_id)):
    """
