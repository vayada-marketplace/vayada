...
@router.post(
    "/images", response_model=MultipleImageUploadResponse, status_code=status.HTTP_201_CREATED
)
async def upload_multiple_images(
    ...
    try:
        ...
        if not uploaded_images:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="No valid images were uploaded"
            )

        return MultipleImageUploadResponse(images=uploaded_images, total=len(uploaded_images))
...
@router.post(
    "/image/hotel-profile", response_model=ImageUploadResponse, status_code=status.HTTP_201_CREATED
)
async def upload_hotel_profile_image(
    file: UploadFile = File(...), user_id: str = Depends(get_current_user_id_allow_pending)
):
    """
...
@router.post(
    "/image/listing", response_model=ImageUploadResponse, status_code=status.HTTP_201_CREATED
)
...
async def upload_listing_images(
    ...
    target_user_id: str | None = Query(
        None,
        description="Target hotel user_id (for admin uploading on behalf of a hotel). When creating a new hotel, first create the hotel to get their user_id, then upload using that user_id here.",
    ),
):
    """
    ...
    **Important for admin creating new hotels:**
    ...
    return await upload_multiple_images(
        files, user_id, prefix="listings", target_user_id=upload_user_id
    )
...
async def upload_creator_profile_image(
    ...
    target_user_id: str | None = Query(
        None,
        description="Target creator user_id (for admin uploading on behalf of a creator). When creating a new user, first create the user to get their user_id, then upload using that user_id here.",
    ),
):
    """
...
async def upload_chat_image(
    ...
    try:
        ...
        if len(file_content) > max_size:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Image must be smaller than 20MB"
            )
