...
from app.models.common import CreatorRequirementsResponse, PlatformResponse
...
async def get_creator_profile_status(user_id: str = Depends(get_current_user_id_allow_pending)):
    ...
    try:
        ...
        profile_complete = len(missing_fields) == 0 and not missing_platforms
...
async def get_creator_profile(user_id: str = Depends(get_current_user_id_allow_pending)):
    ...
    try:
        ...
        average_rating = (
            sum(r["rating"] for r in ratings_data) / total_reviews if total_reviews > 0 else 0.0
        )
...
async def update_creator_profile(
    request: UpdateCreatorProfileRequest, user_id: str = Depends(get_current_user_id_allow_pending)
):
    """
    ...
    try:
        ...
        if (
            request.profilePicture is not None
            and old_profile_picture
            and old_profile_picture != request.profilePicture
        ):
            try:
        ...
        if profile_just_completed:
            try:
                ...
                html_body = create_profile_completion_email_html(
                    user_name, "creator", verification_link
                )
...
async def get_creator_collaborations(
    collab_status: Literal["pending", "accepted", "declined", "completed", "cancelled"]
    | None = Query(None, alias="status", description="Filter by status"),
    initiated_by: Literal["creator", "hotel"] | None = Query(
        None, description="Filter by who initiated"
    ),
...
@router.get(
    "/me/collaborations/{collaboration_id}", response_model=CreatorCollaborationDetailResponse
)
