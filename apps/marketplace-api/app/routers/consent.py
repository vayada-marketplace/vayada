...
async def get_consent_status(user_id: str = Depends(get_current_user_id_allow_pending)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get consent status"
        )
...
async def update_marketing_consent(
    ...
    try:
        ...
        result = await ConsentRepository.update_marketing_consent(
            user_id, request_body.marketing_consent
        )
...
@router.get(
    "/cookies", response_model=Optional[CookieConsentResponse], status_code=status.HTTP_200_OK
)
async def get_cookie_consent(
    visitor_id: str, credentials: HTTPAuthorizationCredentials | None = Depends(optional_security)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get cookie consent"
        )


@router.get("/history", response_model=ConsentHistoryResponse, status_code=status.HTTP_200_OK)
async def get_consent_history(
    user_id: str = Depends(get_current_user_id_allow_pending), limit: int = 50, offset: int = 0
):
    """
    ...
    try:
        ...
        return ConsentHistoryResponse(history=history, total=total)
