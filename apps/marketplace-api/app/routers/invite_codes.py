...
async def create_invite_code(
    ...
    # Ensure unique
    for _ in range(10):
        existing = await Database.fetchval("SELECT 1 FROM invite_codes WHERE code = $1", code)
        if not existing:
            break
        code = _generate_code()
...
async def list_invite_codes(
    ...
    rows = await Database.fetch("SELECT * FROM invite_codes ORDER BY created_at DESC")
...
async def delete_invite_code(
    ...
    result = await Database.execute("DELETE FROM invite_codes WHERE id = $1", invite_id)
    if result == "DELETE 0":
