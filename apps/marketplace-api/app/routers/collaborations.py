...
async def get_collaboration_deliverables(collaboration_id: str) -> list[PlatformDeliverablesItem]:
    ...
    for row in rows:
        ...
        platform_map[platform].append(
            PlatformDeliverable(
                id=str(row["id"]), type=row["type"], quantity=row["quantity"], status=row["status"]
            )
        )
...
async def create_collaboration(
    request: CreateCollaborationRequest, user_id: str = Depends(get_current_user_id)
):
    """
    ...
    try:
        ...
        if request.initiator_type == "creator":
            ...
            if not hotel_profile:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND, detail="Hotel profile not found."
                )
        ...
        creator_record = await CreatorRepository.get_by_id(
            creator_id, columns="id, user_id, profile_picture"
        )
        if not creator_record:
            raise HTTPException(status_code=404, detail="Creator not found")

        creator_user_info = await UserRepository.get_by_id(
            creator_record["user_id"], columns="name, status"
        )
        ...
        platform_deliverables_response = await get_collaboration_deliverables(
        ...
        admin_html = create_admin_collaboration_request_email_html(
            ...
            or (creator_user_info["name"] if creator_user_info else "Unknown"),
...
async def respond_to_collaboration_request(
    ...
    try:
        ...
        if not is_recipient:
            raise HTTPException(
                status_code=403, detail="Not authorized to respond to this collaboration request"
            )
        ...
        query = (
            f"UPDATE collaborations SET {', '.join(updates)} WHERE id = ${len(params)} RETURNING *"
        )
            ...
            async with conn.transaction():
                ...
                if request.status == "accepted":
                    ...
                    await create_system_message(
                        collaboration_id,
                        "💬 Chat is now open! Discuss and finalize the terms.",
                        conn=conn,
                    )
                else:
                    # Declined
        ...
        if initiator_email:
            ...
            subject = (
                "Collaboration Request Accepted" if accepted else "Collaboration Request Declined"
            )
        ...
        if collab["initiator_type"] == "creator":
            ...
            admin_subject = (
                ...
                if accepted
                else "Hotel declined collaboration request"
            )
    ...
    try:
        ...
        if is_now_free_stay:
            if (
                ...
                and current_collab["free_stay_max_nights"] is None
            ):
                target_max = target_min
            elif (
                target_max is not None
                and target_min is None
                and current_collab["free_stay_min_nights"] is None
            ):
        ...
        if request.stay_nights is not None:
            ...
            diff_summary = [
                ...
                if not s.startswith("Min Nights") and not s.startswith("Max Nights")
            ]
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                sys_msg = f"📝 {sender_name} has suggested a counter-offer: " + " • ".join(
                    diff_summary
                )
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                await create_system_message(
                    collaboration_id, f"✅ {sender_name} approved the terms.", conn=conn
                )
                if new_status == "accepted":
                    await create_system_message(
                        collaboration_id, "🎉 Collaboration Accepted!", conn=conn
                    )
        ...
        if new_status == "accepted":
            ...
            for email, name in [
                (creator_email, creator_email_name),
                (hotel_email, hotel_email_name),
            ]:
                if email:
            ...
            if other_email:
                ...
                send_email_background(
                    other_email, "Terms Approved — Your Confirmation Needed", html
                )
...
async def cancel_collaboration(
    ...
    try:
        ...
        if not is_creator and not is_hotel:
            raise HTTPException(
                status_code=403, detail="Not authorized to cancel this collaboration"
            )
        ...
        if collab["status"] in ["completed", "cancelled", "declined"]:
            raise HTTPException(
                status_code=400,
                detail=f"Cannot cancel collaboration with status '{collab['status']}'",
            )

        sender_name = "Creator" if is_creator else "Hotel"

        # Update
        updates = ["status = 'cancelled'", "cancelled_at = NOW()", "updated_at = NOW()"]

        pool = await Database.get_pool()
        async with pool.acquire() as conn:
            async with conn.transaction():
                await conn.execute(
                    f"UPDATE collaborations SET {', '.join(updates)} WHERE id = $1",
                    collaboration_id,
                )
...
@router.post(
    "/{collaboration_id}/deliverables/{deliverable_id}/toggle", response_model=CollaborationResponse
)
async def toggle_deliverable(
    collaboration_id: str, deliverable_id: str, user_id: str = Depends(get_current_user_id)
):
    """
    ...
    try:
        ...
        await create_system_message(
            collaboration_id,
            f"{'✅' if new_status == 'completed' else '🔄'} Deliverable '{deliverable_name}' marked as {status_text}.",
        )
...
async def rate_collaboration(
    ...
    try:
        ...
        if not user or user["type"] != "hotel":
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Only hotels can rate collaborations"
            )
        ...
        if collab["status"] != "completed":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Collaboration is not completed"
            )

        # Check if already rated
        existing_rating = await CollaborationRepository.get_rating_by_collaboration(
            collaboration_id
        )
