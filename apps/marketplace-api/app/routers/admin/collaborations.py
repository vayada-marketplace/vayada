...
async def get_admin_collaborations(
    ...
    search: str | None = Query(None, description="Search by creator name or hotel name"),
    ...
    try:
        ...
        if search:
            search_users = await AuthDatabase.fetch(
                "SELECT id FROM users WHERE name ILIKE $1", f"%{search}%"
            )
        ...
        if search:
            if search_user_ids:
                where_conditions.append(
                    f"(hp.name ILIKE ${param_counter} OR cr.user_id = ANY(${param_counter + 1}::uuid[]))"
                )
        ...
        if rows:
            ...
            user_rows = await AuthDatabase.fetch(
                "SELECT id, name FROM users WHERE id = ANY($1::uuid[])", creator_user_ids
            )
        ...
        for row in rows:
            ...
            collaborations.append(
                CollaborationResponse.from_db_row(
                    row,
...
async def admin_approve_collaboration(
    collaboration_id: str, admin_id: str = Depends(get_admin_user)
):
    """
        ...
        for email, name in [(creator_email, creator_email_name), (hotel_email, hotel_email_name)]:
            if email:
                ...
                html = create_collaboration_approved_email_html(
                    ...
                    collaboration_type=collab["collaboration_type"],
                    listing_name=(await CollaborationRepository.get_full(collaboration_id))[
                        "listing_name"
                    ],
