...
async def get_all_listings():
    ...
    try:
        ...
        for o in offerings_data:
            ...
            offerings_map[listing_id_str].append(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to fetch listings"
        )


@router.get("/creators", response_model=list[CreatorMarketplaceResponse])
async def get_all_creators(
    creator_type: list[Literal["Lifestyle", "Travel"]] | None = Query(
        None, description="Filter by creator type(s)"
    ),
):
    """
    ...
    try:
        ...
        for p in platforms_data:
            ...
            platforms_map[creator_id_str].append(
                PlatformMarketplaceResponse(
                    id=str(p["id"]),
                    name=p["name"],
                    handle=p["handle"],
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to fetch creators"
        )
