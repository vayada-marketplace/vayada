...
def validate_image(
    ...
    try:
        ...
        if settings.IMAGE_RESIZE_WIDTH > 0 or settings.IMAGE_RESIZE_HEIGHT > 0:
            ...
            max_allowed_width = (
                max(settings.MAX_IMAGE_WIDTH, settings.IMAGE_RESIZE_WIDTH * 10)
                if settings.IMAGE_RESIZE_WIDTH > 0
                else settings.MAX_IMAGE_WIDTH
            )
            max_allowed_height = (
                max(settings.MAX_IMAGE_HEIGHT, settings.IMAGE_RESIZE_HEIGHT * 10)
                if settings.IMAGE_RESIZE_HEIGHT > 0
                else settings.MAX_IMAGE_HEIGHT
            )
        else:
        ...
        if width > max_allowed_width or height > max_allowed_height:
            return (
                False,
                f"Image dimensions exceed maximum allowed size ({max_allowed_width}x{max_allowed_height})",
            )
