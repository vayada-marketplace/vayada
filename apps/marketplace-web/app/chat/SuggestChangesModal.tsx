...
export default function SuggestChangesModal({
  ...
  const getInitialCollaborationType = () => {
    if (
      userType === "creator" &&
      allowedCollaborationTypes &&
      allowedCollaborationTypes.length > 0
    ) {
      // If initial type is allowed, use it; otherwise use the first allowed type
      if (
        initialCollaborationType &&
        allowedCollaborationTypes.includes(initialCollaborationType)
      ) {
            ...
            <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 space-y-4">
              <div>
                ...
                allowedCollaborationTypes.length > 0 ? (
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setCollaborationType(
                          e.target.value as "Free Stay" | "Paid" | "Discount" | "Affiliate",
                        )
                      }
                  ...
                  <select
                    value={collaborationType}
                    onChange={(e) =>
                      setCollaborationType(e.target.value as "Free Stay" | "Paid" | "Discount")
                    }
                    className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-900 focus:outline-none focus:border-blue-500"
                  >
              ...
              {collaborationType === "Free Stay" && (
                <div>
                  ...
                  <div className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl px-4 py-2">
                    <button
                      onClick={() => setFreeStayMaxNights((prev) => Math.max(1, prev - 1))}
                    ...
                    <button
                      onClick={() => setFreeStayMaxNights((prev) => prev + 1)}
                      className="text-gray-400 hover:text-gray-900"
                    >
              ...
              {collaborationType === "Affiliate" && (
                <div>
                  ...
                  <div className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl px-4 py-2">
                    <button
                      onClick={() => setCreatorFee((prev) => Math.max(0, prev - 1))}
                      className="text-gray-400 hover:text-gray-900"
                    >
                      <MinusIcon className="w-5 h-5" />
                    </button>
                    ...
                    <button
                      onClick={() => setCreatorFee((prev) => Math.min(100, prev + 1))}
                      className="text-gray-400 hover:text-gray-900"
                    >
                      <PlusIcon className="w-5 h-5" />
                    </button>
                    ...
                    {pd.deliverables.map((d, dIndex) => (
                      <div key={dIndex} className="flex items-center gap-3">
                        <input
                          ...
                          value={d.type}
                          onChange={(e) =>
                            handleDeliverableChange(pIndex, dIndex, "type", e.target.value)
                          }
                        ...
                        <div className="flex items-center gap-2.5 bg-white border border-gray-200 rounded-lg px-2 py-1.5">
                          <button
                            onClick={() => adjustQuantity(pIndex, dIndex, -1)}
                            className="text-gray-400 hover:text-gray-900 transition-colors"
                          >
                            <MinusIcon className="w-4 h-4" />
                          </button>
                          <span className="text-sm font-bold text-gray-900 min-w-[16px] text-center">
                            {d.quantity}
                          </span>
                          <button
                            onClick={() => adjustQuantity(pIndex, dIndex, 1)}
                        ...
                        <button
                          onClick={() => removeDeliverable(pIndex, dIndex)}
