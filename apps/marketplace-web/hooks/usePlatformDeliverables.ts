...
export function usePlatformDeliverables() {
    ...
    setPlatformDeliverables((prev: PlatformDeliverable[]) =>
      prev.map((pd: PlatformDeliverable) => {
        ...
        if (existing) {
          return {
            ...pd,
            deliverables: pd.deliverables.filter(
              (d: DeliverableItem) => d.type !== deliverableType,
            ),
          };
        } else {
          return {
  ...
  const handleDeliverableQuantityChange = useCallback(
    (platform: string, deliverableType: string, quantity: number) => {
      ...
      prev
        .map((pd: PlatformDeliverable) =>
          pd.platform === "Custom"
            ? {
                ...pd,
                deliverables: pd.deliverables.filter((d: DeliverableItem) => d.type !== type),
              }
            : pd,
        )
        .filter(
          (pd: PlatformDeliverable) => pd.platform !== "Custom" || pd.deliverables.length > 0,
        ),
    );
  }, []);
  ...
  const getPlatformDeliverables = useCallback(
    (platform: string) => {
