...
export function OfferingEditorCard({
  ...
  // backend validator doesn't reject the request. Commission has no default.
  const setType = (next: CollaborationKind) => {
    const nextCommission = next === "Affiliate" ? offering.commissionPercentage : undefined;
    onChange({
      ...offering,
      type: next,
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm space-y-4">
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mb-2">
          Overrides the listing-level minimum for this offering only — e.g. require 100k+ for a Free
          Stay but 10k for a Discount.
        </p>
        <Input
          type="number"
          min={1}
