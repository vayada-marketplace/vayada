...
import {
  superAdminPayoutsService,
  AffiliatePayoutSummary,
  AffiliatePayoutDetail,
} from '@/services/superAdminPayouts'
    ...
    <div className="p-4 md:p-8 max-w-[1400px]">
              ...
              {filtered.map((a) => (
                <tr key={a.affiliateId} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-right font-semibold text-gray-900">
                    ...
                    {a.unpaidCount > 0 && (
                      <span className="block text-[10px] text-gray-400 font-normal">{a.unpaidCount} bookings</span>
                    )}
                  </td>
      ...
      {selectedId && (
        <PayoutDetailDrawer
          ...
          onPaid={() => { setSelectedId(null); fetchList() }}
        />
      )}
    </div>
  )
...
function PayoutDetailDrawer({
  ...
  useEffect(() => {
    setLoading(true)
    superAdminPayoutsService.get(affiliateId)
      .then(setDetail)
  ...
  const handleMarkPaid = async () => {
    ...
    setError('')
    try {
      await superAdminPayoutsService.markPaid(affiliateId, {
            ...
            <section>
              ...
              {detail.lines.length === 0 ? (
                <p className="text-[13px] text-gray-500 bg-gray-50 border border-gray-100 rounded-lg p-4">
                  No commission rows yet. Bookings show up here once they get scheduled into payouts.
                </p>
              ) : (
                  ...
                  {detail.history.map((h, i) => (
                    <div key={i} className="px-4 py-3 text-[12px]">
                      <div className="flex items-center justify-between">
                        ...
                        {h.externalReference && (
                          <p className="text-[10px] font-mono text-gray-400">{h.externalReference}</p>
                        )}
                      </div>
