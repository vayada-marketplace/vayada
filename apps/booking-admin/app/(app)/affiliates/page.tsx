...
export default function AffiliatesPage() {
  ...
  useEffect(() => {
    settingsService
      .getPropertySettings()
      .then((s) => {
        if (s.default_currency) setCurrencySymbol(getCurrencySymbol(s.default_currency));
      })
      .catch(() => {});
  }, []);
                ...
                {pendingAffiliates.map((a) => (
                  <div key={a.id} className="bg-white border border-gray-200 rounded-xl p-3.5">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      <span
                        className={`shrink-0 inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium capitalize ${USER_TYPE_STYLES[a.userType] || ""}`}
                      >
                        {a.userType}
                      </span>
                    </div>
                    <div className="text-[12px] text-gray-500 space-y-0.5 border-t border-gray-100 pt-2 mb-3">
                      {a.socialMedia && (
                        <p>
                          Channel: <span className="text-gray-700">{a.socialMedia}</span>
                        </p>
                      )}
                      <p>
                        Payout:{" "}
                        <span className="text-gray-700">{getPayoutLabel(a.paymentMethod)}</span>
                      </p>
                      <p>
                        Applied: <span className="text-gray-700">{formatDate(a.createdAt)}</span>
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <button
                    ...
                    {pendingAffiliates.map((a) => (
                      <tr key={a.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-4 py-3 text-gray-600">
                          {getPayoutLabel(a.paymentMethod)}
                        </td>
                  ...
                  <div key={a.id} className="bg-white border border-gray-200 rounded-xl p-3.5">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      <div className="flex flex-col items-end gap-1 shrink-0">
                        <span
                          className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium capitalize ${STATUS_STYLES[a.status] || ""}`}
                        >
                          {a.status === "suspended" ? "Blocked" : a.status}
                        </span>
                        <span
                          className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium capitalize ${USER_TYPE_STYLES[a.userType] || ""}`}
                        >
                          {a.userType}
                        </span>
                      </div>
                    ...
                    <div className="border-t border-gray-100 pt-2 flex gap-2">
                      <button
                        onClick={() => setPayoutModalAffiliate(a)}
                        className="flex-1 py-1.5 text-[12px] font-medium text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
                      >
                        View payout
                      </button>
                      {a.status === "approved" ? (
                      ...
                      <tr key={a.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-4 py-3 text-right font-medium text-gray-900">
                          ...
                          {a.commissionPctOverride !== null && (
                            <span
                              className="ml-1 inline-flex px-1.5 py-0 rounded-full text-[9px] font-semibold bg-amber-50 text-amber-700"
                              title="Custom rate — click Set rate to change or revert"
                            >
                              Custom
                            </span>
                          )}
    ...
    >
      ...
      >
        <h3 className="text-[14px] font-semibold text-gray-900">
          Commission rate for {affiliate.fullName}
        </h3>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {method === "stripe" && (
            <>
              ...
              <Field
                label="Onboarded"
                value={affiliate.stripeConnectOnboarded ? "Yes" : "Not yet"}
              />
            </>
          )}
    ...
    <div className="mb-4 bg-white border border-gray-200 rounded-lg p-4 flex items-center justify-between flex-wrap gap-3">
      <div>
        ...
        <p className="text-[12px] text-gray-500">
          Applied to every new affiliate unless overridden for a specific one.
        </p>
      </div>
      <div className="flex items-center gap-2">
        {editing === null ? (
