...
export default function LastMinuteTab() {
  ...
  const addTier = () => {
    setConfig((prev) => ({
      ...prev,
      tiers: [...prev.tiers, { daysBeforeMin: 0, daysBeforeMax: null, discountPercent: 10 }],
    }));
  };
  ...
  return (
    <div className="space-y-4">
      {feedback && (
        <div
          className={`p-3 rounded-lg text-sm ${feedback.type === "success" ? "bg-green-50 border border-green-200 text-green-700" : "bg-red-50 border border-red-200 text-red-700"}`}
        >
          {feedback.message}
        </div>
      )}
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4 md:p-5">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() =>
                  setConfig((prev) => ({ ...prev, stackWithPromo: !prev.stackWithPromo }))
                }
