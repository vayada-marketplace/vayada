...
export function LodgifyIntegrationPanel() {
  ...
  const onConnect = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setApiKey('')
      setPropertyId('')
      setFeedback({ type: 'success', message: 'Lodgify connected. Rates and rooms will start syncing in a future release.' })
    } catch (err) {
  ...
  const onDisconnect = async () => {
    if (!confirm('Disconnect Lodgify? Your stored API key will be cleared. You can reconnect anytime.')) return
    setSubmitting(true)
    setFeedback(null)
    try {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-[13px] text-gray-500 mt-0.5">
              Connect your Lodgify property so the Vayada Booking Engine
              can read rooms, rates and availability from Lodgify.
            </p>
          </div>
      ...
      <p className="text-[12px] text-gray-400">
        Phase 1a: connection only. Rates/availability sync and booking
        write-back ship in follow-up releases.
      </p>
    </div>
  )
}
