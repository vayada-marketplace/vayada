...
class HotelRepository:
    @staticmethod
    async def get_by_id(hotel_id: str) -> Optional[dict]:
        row = await Database.fetchrow("SELECT * FROM hotels WHERE id = $1", hotel_id)
        return dict(row) if row else None

    @staticmethod
    async def get_owned_id(hotel_id: str, user_id: str) -> Optional[str]:
    ...
    @staticmethod
    async def get_oldest_for_user(user_id: str) -> Optional[dict]:
    ...
    async def create(
        ...
        if hotel_id:
            row = await Database.fetchrow(
                ...
                hotel_id, slug, name, contact_email, user_id,
            )
        else:
            row = await Database.fetchrow(
    ...
    @staticmethod
    async def update_fields(hotel_id: str, fields: dict) -> Optional[dict]:
        ...
        sql = (
            f"UPDATE hotels SET {', '.join(set_clauses)} "
    ...
    async def update_benefits(hotel_id: str, benefits: list) -> None:
        await Database.execute(
            ...
            json.dumps(benefits), hotel_id,
        )

    @staticmethod
    async def get_guest_form_settings(hotel_id: str) -> Optional[dict]:
        row = await Database.fetchrow(
