...
def test_validate_attachment_accepts_image_jpeg():
    validate_attachment(
        content_type="image/jpeg", size_bytes=1024, channel="airbnb",
    )


def test_validate_attachment_accepts_pdf():
    validate_attachment(
        content_type="application/pdf", size_bytes=1024, channel="booking.com",
    )
...
def test_validate_attachment_strips_charset_suffix():
    ...
    validate_attachment(
        ...
        size_bytes=1024, channel="airbnb",
    )


def test_validate_attachment_is_case_insensitive_on_mime():
    validate_attachment(
        content_type="IMAGE/PNG", size_bytes=1024, channel="airbnb",
    )
...
def test_validate_attachment_rejects_zip():
    with pytest.raises(HTTPException) as exc:
        validate_attachment(
            ...
            size_bytes=1024, channel="booking.com",
        )
...
def test_validate_attachment_rejects_missing_content_type():
    with pytest.raises(HTTPException) as exc:
        validate_attachment(
            content_type=None, size_bytes=1024, channel="airbnb",
        )
    assert exc.value.status_code == 415
...
def test_validate_attachment_rejects_oversize_for_bdc():
    ...
    with pytest.raises(HTTPException) as exc:
        validate_attachment(
            ...
            size_bytes=9 * 1024 * 1024, channel="booking.com",
        )
...
def test_validate_attachment_allows_bdc_at_limit():
    validate_attachment(
        content_type="image/jpeg",
        size_bytes=8 * 1024 * 1024, channel="booking.com",
    )
...
def test_validate_attachment_allows_airbnb_up_to_25mb():
    validate_attachment(
        content_type="image/jpeg",
        size_bytes=25 * 1024 * 1024, channel="airbnb",
    )
...
def test_validate_attachment_rejects_airbnb_above_25mb():
    with pytest.raises(HTTPException) as exc:
        validate_attachment(
            content_type="image/jpeg",
            size_bytes=26 * 1024 * 1024, channel="airbnb",
        )
    assert exc.value.status_code == 413
...
async def test_upload_rejects_disallowed_mime_before_calling_channex():
    ...
    with patch.object(admin_messaging, "get_hotel_id", AsyncMock(return_value="h1")), \
         patch.object(
             admin_messaging.MessageThreadRepository, "get_by_id",
             AsyncMock(return_value={"channel": "booking.com", "source_thread_id": "t-src"}),
         ), \
         patch.object(
             admin_messaging.channex_service, "upload_attachment",
        ...
        with pytest.raises(HTTPException) as exc:
            await upload_thread_attachment(
                ...
                file=_FakeUploadFile(
                    b"data", content_type="application/zip", filename="x.zip",
                ),
                user_id="u1",
            )
    assert exc.value.status_code == 415
    ...
    with patch.object(admin_messaging, "get_hotel_id", AsyncMock(return_value="h1")), \
        ...
        with pytest.raises(HTTPException) as exc:
            await upload_thread_attachment(
                thread_id="t1",
                file=_FakeUploadFile(
                    big, content_type="image/jpeg", filename="x.jpg",
                ),
                user_id="u1",
            )
    assert exc.value.status_code == 413
...
async def test_upload_calls_channex_for_allowed_jpeg():
    ...
    with patch.object(admin_messaging, "get_hotel_id", AsyncMock(return_value="h1")), \
         ...
         patch.object(
             admin_messaging.channex_service, "get_platform_api_key",
             return_value="key",
         ):
        result = await upload_thread_attachment(
            thread_id="t1",
            file=_FakeUploadFile(
                b"jpegdata", content_type="image/jpeg", filename="ok.jpg",
            ),
            user_id="u1",
        )
...
async def test_send_message_with_two_attachments_issues_two_channex_calls():
    ...
    insert_mock = AsyncMock(side_effect=[
    ...
    with patch.object(admin_messaging, "get_hotel_id", AsyncMock(return_value="h1")), \
         patch.object(
             ...
             AsyncMock(return_value={
                 "id": "t1", "channel": "booking.com",
                 "source_thread_id": "t-src",
             }),
         ), \
         patch.object(
             admin_messaging.MessageRepository, "insert_and_update_thread",
             insert_mock,
         ), \
         patch.object(
             admin_messaging.MessageAttachmentRepository, "add",
             AsyncMock(),
         ), \
         patch.object(
             admin_messaging.MessageAttachmentRepository, "list_by_message",
             AsyncMock(return_value=[]),
         ), \
         patch.object(
             admin_messaging.channex_service, "post_thread_message", post_mock,
         ), \
         patch.object(
             admin_messaging.channex_service, "get_platform_api_key",
             return_value="key",
