...
export function Avatar({ src, alt, name, className = "", size = "md" }: AvatarProps) {
  ...
  useEffect(() => {
    ...
    if (src && !src.startsWith("http") && !src.startsWith("data:")) {
      ...
      if (src.startsWith("/")) {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || "https://api.localhost";
  ...
  if (error || !imgSrc) {
    ...
    return (
      <div
        className={`rounded-full bg-gray-200 flex items-center justify-center text-gray-400 ${sizeClasses[size]} ${className}`}
      >
