...
export const consentService = {
  ...
  getCookieConsent: async (visitorId: string): Promise<CookieConsentResponse | null> => {
    try {
      return await apiClient.get<CookieConsentResponse>(
        `/consent/cookies?visitor_id=${encodeURIComponent(visitorId)}`,
      );
    } catch {
      return null;
    }
  ...
  getConsentHistory: async (limit = 50, offset = 0): Promise<ConsentHistoryResponse> => {
    return apiClient.get<ConsentHistoryResponse>(
      `/consent/history?limit=${limit}&offset=${offset}`,
    );
  },
  ...
  downloadExport: async (token: string): Promise<Blob> => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/gdpr/export-download?token=${encodeURIComponent(token)}`,
      {
        headers: {
