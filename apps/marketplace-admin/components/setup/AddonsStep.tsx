      ...
      <div className="max-w-2xl mx-auto px-6 py-6">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-[13px] text-gray-500 mt-1">
            Pre-configure extras like airport transfers, spa treatments, or breakfast packages
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-5 mb-4">
          ...
          {addons.length === 0 ? (
            <div className="bg-gray-50 rounded-lg border border-dashed border-gray-300 p-6 text-center">
              ...
              <p className="text-[12px] text-gray-400 mt-0.5">Add guest experiences to pre-configure for this hotel</p>
            </div>
          ) : (
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
          <p className="text-[12px] text-blue-700">
            This step is optional — the hotel can also configure add-ons after setup.
          </p>
        </div>
        ...
        <div className="mt-6 flex justify-between">
          ...
          >
            {addons.length === 0 ? 'Skip' : 'Continue'} &rarr;
          </button>
        </div>
      </div>
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 max-h-[85vh] overflow-y-auto">
            ...
            <div className="p-4 space-y-3">
              ...
              <div>
                ...
                {formData.image ? (
                  <div className="relative rounded-lg overflow-hidden bg-gray-200">
                    <img src={formData.image} alt="Add-on" className="w-full h-36 object-cover" onError={(e) => { e.currentTarget.style.display = 'none' }} />
                    <button
                      type="button"
              ...
              <div className="flex items-center justify-between py-1">
                <div>
                  <p className="text-[12px] font-medium text-gray-700">Per Person Pricing</p>
                  <p className="text-[11px] text-gray-400">Price is multiplied by number of guests</p>
                </div>
                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, perPerson: !formData.perPerson })}
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                    formData.perPerson ? 'bg-primary-500' : 'bg-gray-300'
                  }`}
                >
                  <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${
                    formData.perPerson ? 'translate-x-[18px]' : 'translate-x-[3px]'
                  }`} />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between py-1">
                <div>
                  <p className="text-[12px] font-medium text-gray-700">Per Night Pricing</p>
                  <p className="text-[11px] text-gray-400">Price is charged for each night of the stay</p>
                </div>
                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, perNight: !formData.perNight })}
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                    formData.perNight ? 'bg-primary-500' : 'bg-gray-300'
                  }`}
                >
                  <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${
                    formData.perNight ? 'translate-x-[18px]' : 'translate-x-[3px]'
                  }`} />
                </button>
              </div>
            </div>
