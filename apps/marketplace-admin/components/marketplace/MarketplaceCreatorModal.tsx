...
import { MapPinIcon, StarIcon, GlobeAltIcon, UserGroupIcon } from "@heroicons/react/24/outline";
        ...
        <div>
          ...
          <div className="space-y-4">
            {creator.platforms.map((platform) => {
              ...
              const hasDemographics = countries.length > 0 || ageGroups.length > 0 || !!genderSplit;
              return (
                <div key={platform.id} className="p-4 border rounded-lg">
                  <div className="flex items-center justify-between mb-3">
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-medium ${getPlatformColor(platform.name)}`}
                    >
                      {platform.name}
                    </span>
