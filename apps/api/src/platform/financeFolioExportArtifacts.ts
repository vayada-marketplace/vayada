...
import { FINANCE_FOLIO_CSV_CONTENT_TYPE, FINANCE_FOLIO_CSV_VERSION } from "@vayada/domain-finance";
...
export function createS3FinanceFolioExportArtifactWriter(config: { bucketName: string; s3Client?: S3Client }): FinanceFolioExportArtifactWriter {
  ...
  return {
    ...
    async write(input) {
      if (!uuid(input.exportId) || input.contentType !== FINANCE_FOLIO_CSV_CONTENT_TYPE || !instant(input.expiresAt)) throw new TypeError("Invalid Finance folio export artifact");
      const bytes = Buffer.from(input.body, "utf8"), digest = createHash("sha256").update(bytes).digest(), storageKey = `private/finance/financials-exports/${input.exportId}/${FINANCE_FOLIO_CSV_VERSION}.csv`;
