...
describe("pending Channex upload batch continuation", () => {
  it("handles ten of eleven candidates, then skips completed attempts on continuation", async () => {
    ...
    owners.read.mockResolvedValue({ kind: "available", authority: { lease: { propertyId: "property" } } });
    ...
    const query = vi.fn(async (_sql, values) => {
      ...
      return { rows: [...pending].map((attemptId) => ({
        attemptId, creationAttemptId: `creation-${attemptId}`, roomTypeId: "room",
        offerId: "offer", operationKey: "operation", primaryOccupancy: 2,
      })) };
    });
    ...
    expect(await reconcilePendingChannexUploads(pool, lease, get)).toEqual({
      kind: "unavailable", reason: "reconciliation_batch_pending",
    });
    expect(owners.reconcile).toHaveBeenCalledTimes(10);
    expect(pending.size).toBe(1);
    expect(await reconcilePendingChannexUploads(pool, lease, get)).toEqual({
      kind: "pending_uploads_reconciled", count: 1,
    });
    expect(owners.reconcile).toHaveBeenCalledTimes(11);
    expect(await reconcilePendingChannexUploads(pool, lease, get)).toEqual({
      kind: "pending_uploads_reconciled", count: 0,
    });
    expect(owners.reconcile).toHaveBeenCalledTimes(11);
  });
});
