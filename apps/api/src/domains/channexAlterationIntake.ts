...
export async function persistChannexAlteration(
  ...
  try {
    ...
    const result = await persistChannexAlterationInTransaction(client, expected, value);
...
/** Caller owns transaction and must commit the request with its durable job cursor. */
export async function persistChannexAlterationInTransaction(
