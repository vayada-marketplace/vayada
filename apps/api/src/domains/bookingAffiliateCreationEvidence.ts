...
export async function readBookingAffiliateCreationEvidence(
  ...
  const result = await database.query(
    `SELECT b.source_system,b.source_booking_id,b.booking_channel,
      ...
      e.id AS event_id,e.actor_type,e.from_status,e.to_status,e.occurred_at,
      e.event_payload->>'requestId' AS request_id,
  ...
  if (
    ...
    !["draft", "pending_payment", "confirmed"].includes(row.to_status) ||
