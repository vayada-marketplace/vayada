...
import {
  ...
  activateBookingPricingDraftTerms,
...
import {
  lockReplacementPricingDraftOwners,
  lockReplacementPricingOfferOwners,
...
export async function lockReplacementPricingSources(
  client: PoolClient,
  context: RequestContext | null,
  scope: PricingStorageScope,
  access: "read" | "manage" | "preview",
): Promise<PricingStorageSources | null> {
...
export function createReplacementPricingStorageGuard(
  context: RequestContext | null,
): PricingStorageGuard {
  const trustedContext = structuredClone(context);
  return {
    lock: (client, scope, access) =>
      lockReplacementPricingSources(client, trustedContext, scope, access),
    async project(client, scope, proposed, sources, draft, access) {
      const projected = await projectBookingPricingDraftTerms(
        client,
        trustedContext,
        scope,
        draft,
        proposed.rooms.flatMap((r) =>
          r.offers.map((o) => ({
    ...
    async activate(client, scope, proposed, sources, draft) {
      await activateBookingPricingDraftTerms(
        client,
        trustedContext,
        scope,
        draft,
    ...
    async validate(client, scope, proposed, sources, intent, draft) {
      if (
        Object.keys(proposed.ownerReferences).some(
          (key) => !["finance", "charges"].includes(key),
        ) ||
        Object.keys(sources).length !== 3 ||
        !["room", "terms", "finance"].every((key) => typeof sources[key] === "string")
      )
        return false;
      const result = await (
        intent === "draft" ? lockReplacementPricingDraftOwners : lockReplacementPricingOfferOwners
      )(client, trustedContext, scope, proposed, sources, draft);
      return (
        result.kind === "verified" ||
        (intent === "draft" && result.kind === "awaiting_charge_confirmation")
      );
    },
    async allowCurrencyChange() {
      return false;
    },
  };
}
