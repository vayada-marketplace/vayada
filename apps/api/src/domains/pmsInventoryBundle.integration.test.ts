...
describe.skipIf(!url)("mixed room inventory transactions", () => {
  ...
  it("discovers and reprices complete selections from canonical full-stay evidence", async () => {
    ...
    const offer = publicRoomCombinationOffer(
      result.options[0]!,
      "https://example.test/en/book?checkIn=2027-02-01&checkOut=2027-02-03&adults=5&children=1",
    );
    ...
    expect(
      publicRoomCombinationOffer(result.options[0]!, "https://example.test/en/book").offerId,
    ).toBe(serialized.offerId);

    expect((await search(4, 0)).options.every((option) => option.lines.length === 1)).toBe(true);
    expect(await search(9, 0)).toMatchObject({
      complete: true,
      eligibleOfferCount: 2,
      options: [],
      unavailableReasons: [{ code: "occupancy_unavailable" }],
    });
    expect(await search(5, 1, 1)).toMatchObject({
      complete: false,
  ...
  it.each([
    [
      "available_rooms=0, availability_status='sold_out', sellable_publicly=false",
      "sold_out",
      true,
    ],
    ...
    try {
      await client.query("BEGIN");
      await client.query(
        `UPDATE distribution.public_room_offer_snapshots SET ${update} WHERE property_id=$1`,
        [propertyId],
      );
      expect(
        await findTargetRoomCombinationOffers(client, {
          ...input,
          adults: 5,
          children: 1,
          today: "2027-01-01",
  ...
  it("distinguishes incompatible methods across otherwise sufficient room types", async () => {
    ...
    try {
      await client.query("BEGIN");
      await client.query(
        "UPDATE distribution.public_room_offer_snapshots SET payment_options=ARRAY['card'] WHERE property_id=$1 AND room_type_id=$2",
        [propertyId, rooms[0]],
      );
      expect(
        await findTargetRoomCombinationOffers(client, {
          ...
          today: "2027-01-01",
          requestedAt: input.occurredAt,
          paymentMethods: ["card", "pay_at_property"],
        }),
      ).toMatchObject({
        complete: true,
        options: [],
  ...
  it.each([
    ...
    try {
      await client.query("BEGIN");
      await client.query(
        `INSERT INTO pms.rate_rules(property_id,room_type_id,rule_type,starts_on,ends_on,${column}) VALUES($1,$2,'stay_restriction',$3,$3,$4)`,
        [propertyId, rooms[0], input.checkIn, value],
      );
      expect(
        await findTargetRoomCombinationOffers(client, {
          ...input,
  ...
  it.each([
    ...
    try {
      await client.query("BEGIN");
      await client.query(
        `INSERT INTO pms.rate_rules(property_id,room_type_id,rule_type,starts_on,ends_on,stop_sell,enabled)
        VALUES($1,$2,'stay_restriction',$3,$3,true,$4)`,
        [propertyId, rooms[0], day, enabled],
      );
      const result = await findTargetRoomCombinationOffers(client, {
        ...input,
        adults: 5,
        children: 1,
        today: "2027-01-01",
  ...
  it("routes complete public selections only behind the gate and binds links and expiry", async () => {
    ...
    try {
      await client.query("BEGIN");
      await client.query(
        "UPDATE distribution.public_hotel_bookability_profiles SET expires_at='2027-01-01T10:05:00Z' WHERE property_id=$1",
        [propertyId],
      );
      await client.query(
      ...
      const fixture = structuredClone(
        PUBLIC_BOOKABILITY_FIXTURES.find((item) => item.caseId === "bookable")!.profile,
      );
      Object.assign(fixture.hotel, {
        propertyId,
        slug: propertyId,
        timezone: "Europe/Athens",
        bookingBaseUrl: "https://example.test",
      });
      const queries: string[] = [];
      const failures: string[] = [];
      const repository = (enabled = false) =>
        createTargetPublicHotelQuoteRepository({
          connectionString: url!,
          mixedRoomSelectionsEnabled: enabled,
      ...
      expect(await repository().findQuoteBySlug(propertyId, request)).toMatchObject({
        status: "unavailable",
        unavailableReasons: [{ code: "occupancy_unavailable" }],
      });
      queries.length = 0;
      const projection = serializePublicHotelQuoteProjection(
        (await repository(true).findQuoteBySlug(propertyId, request))!,
      );
      ...
      expect(Object.fromEntries(link.searchParams)).toMatchObject({
        ...
        children: "1",
        rooms: "3",
        room: projection.quote!.offers[0].offerId,
      });
      const fourRooms = await repository(true).findQuoteBySlug(propertyId, {
        ...request,
        rooms: "4",
      });
      expect(fourRooms?.status).toBe("bookable");
      expect(
        fourRooms?.quote?.offers.every(
          (offer) => offer.roomLines!.reduce((sum, line) => sum + line.roomCount, 0) >= 4,
        ),
      ).toBe(true);
      await client.query(
      ...
      expect(await repository(true).findQuoteBySlug(propertyId, request)).toMatchObject({
        status: "unavailable",
        unavailableReasons: [{ code: "payment_disabled" }],
      });
    } finally {
  ...
  it("uses the minimum occupancy over every night and requires every explicit bound", async () => {
    ...
    try {
      await client.query("BEGIN");
      await client.query(
        "UPDATE distribution.public_room_offer_snapshots SET occupancy=jsonb_set(occupancy,'{maxAdults}','1') WHERE property_id=$1 AND stay_date='2027-02-02'",
        [propertyId],
      );
      const run = () =>
        findTargetRoomCombinationOffers(client, {
          ...input,
          adults: 5,
          children: 0,
          today: "2027-01-01",
      ...
      await client.query(
        "UPDATE distribution.public_room_offer_snapshots SET occupancy=occupancy-'maxChildren' WHERE property_id=$1",
        [propertyId],
      );
      ...
      for (const bound of ['"unknown"', "99999999999999999999999999999"]) {
        await client.query(
          "UPDATE distribution.public_room_offer_snapshots SET occupancy=jsonb_set(occupancy,'{maxChildren}',$2::jsonb) WHERE property_id=$1",
          [propertyId, bound],
        );
        expect(await run()).toMatchObject({ complete: false, eligibleOfferCount: 0, options: [] });
      }
  ...
  it("credits complete reserved bundles without assuming receipt order or an original quote UUID", async () => {
    ...
    try {
      await client.query("BEGIN");
      const reservation = await port.reserveBundle!({
        ...input,
        transaction: client,
        quoteSessionId: `change-request:${randomUUID()}`,
      });
      expect(
        (
          await client.query(
      ...
      const base = {
        ...input,
        transaction: client,
        reservation: { ...reservation, receipts: [...reservation.receipts].reverse() },
      };
      expect(await port.bundleAvailabilityCredits!(base)).toEqual(
        new Map(
          rooms.map((id) => [
            id,
            { checkIn: input.checkIn, checkOut: input.checkOut, roomCount: 2 },
          ]),
        ),
      );
      expect(
        await port.bundleAvailabilityCredits!({ ...base, propertyId: randomUUID() }),
      ).toBeNull();
      expect(await port.bundleAvailabilityCredits!({ ...base, checkOut: "2027-02-04" })).toBeNull();
      expect(
        await port.bundleAvailabilityCredits!({
          ...base,
          lines: input.lines.map((line) => ({ ...line, roomCount: 1 })),
        }),
      ).toBeNull();
      expect(
        await port.bundleAvailabilityCredits!({
          ...base,
          lines: [input.lines[0]!],
          reservation: { ...reservation, receipts: [reservation.receipts[0]!] },
        }),
      ).toBeNull();
      ...
      await client.query("SET LOCAL session_replication_role=replica");
      await client.query(
        "UPDATE pms.inventory_reservation_receipts SET room_type_id=$2::text::uuid,public_offer_key=$2::text WHERE receipt_id=$1",
        [reservation.receipts[1]!.receiptId, rooms[0]],
      );
  ...
  it.each([
    ...
    try {
      ...
      const code = change.startsWith("occupancy=")
        ? "unavailable_data"
        : change.startsWith("freshness_status=")
          ? "stale_data"
          : change.startsWith("available_rooms=")
            ? "sold_out"
            : change.startsWith("rate_summary=")
              ? "min_stay_not_met"
              : "payment_disabled";
      expect(await search()).toMatchObject({
        complete: !["unavailable_data", "stale_data"].includes(code),
        options: [],
        unavailableReasons: [{ code }],
      });
    } finally {
      await pool.query(
    ...
    async (column) => {
      ...
      try {
        ...
        expect(await search()).toMatchObject({
          complete: true,
          options: [],
          unavailableReasons: [{ code: "stay_restricted" }],
        });
      } finally {
  ...
  it.each(["host", "guest"])(
    ...
    async (source) => {
      ...
      try {
        ...
        await client.query("INSERT INTO identity.users(id,email,status) VALUES($1,$2,'active')", [
          context.actorUserId,
          `${context.actorUserId}@example.test`,
        ]);
        await client.query(
          `UPDATE distribution.public_room_offer_snapshots SET public_policy=jsonb_build_object(
        'type','free_until_days_before_arrival','freeCancellationDeadlineDays',CASE WHEN room_type_id=$2 THEN 3 ELSE 7 END,
        'afterDeadlinePenalty','full_booking_amount','noShowPenalty','full_booking_amount') WHERE property_id=$1`,
          [propertyId, rooms[0]],
        );
        const quote = await createTargetMixedCheckoutQuote(
          client,
          property,
          request,
          input.occurredAt,
        );
        const created = await createTargetGuestBooking(
          client,
          port,
          property,
        ...
        const preview = await bookingOwner.previewDates(
          client,
          port,
          property,
          booking,
        ...
        await client.query(
          'UPDATE distribution.public_room_offer_snapshots SET public_policy=\'{"type":"non_refundable"}\' WHERE property_id=$1 AND room_type_id=$2',
          [propertyId, rooms[1]],
        );
        const changed = await bookingOwner.previewDates(
          client,
          port,
          property,
          booking,
        ...
        expect(() => bookingOwner.assertDatesUnchanged(preview, changed)).toThrow(
        ...
        const oldReceipt = booking.bookingMetadata as {
          inventoryReservation: PmsInventoryReservationBundle;
        };
        await port.release({
          transaction: client,
          propertyId,
          reservation: oldReceipt.inventoryReservation,
          occurredAt: input.occurredAt,
        });
        const revision = randomUUID();
        const receipt = await bookingOwner.reserveDates(port, selectedOffer, {
          transaction: client,
          propertyId,
          quoteSessionId: `${source === "host" ? "host-edit" : "change-request"}:${revision}`,
          roomTypeId: rooms[0]!,
          publicOfferKey: rooms[0]!,
        ...
        VALUES($1,$2,'date_change','guest','pending',$3::jsonb)`,
            [revision, booking.guestBookingId, JSON.stringify(preview)],
          );
        const updated = await bookingOwner.applyDates(client, {
          booking,
          changeRequest:
            source === "host"
              ? { id: revision, hostEdit: true }
              : {
                  id: revision,
                  guestBookingId: booking.guestBookingId,
                  status: "pending",
                  requestedChanges: preview,
                  decisionNote: null,
                  decidedAt: null,
        ...
        const pms = await createTargetPmsOperationsReadRepository({
          connectionString: url!,
          pool: client,
        }).findReservationByGuestBookingId(propertyId, updated.guestBookingId);
        expect(pms?.roomLines?.map((line) => line.totals["totalAmount"])).toEqual([
          "200.00",
          "100.00",
        ]);
        // A subsequent change must credit the latest bundle, not the original checkout quote.
        expect(
          await bookingOwner.previewDates(
            client,
            port,
            property,
            updated,
        ...
        const assign = async () => {
          const receipts = (
            await client.query(
              "SELECT receipt_id,room_type_id FROM pms.inventory_reservation_receipts WHERE property_id=$1 AND quote_session_id=$2",
              [propertyId, `${source === "host" ? "host-edit" : "change-request"}:${revision}`],
            )
          ).rows;
          let position = 0;
          for (const line of selection.lines)
            for (const guest of line.guests) {
              ...
              await client.query(
                "INSERT INTO pms.rooms(id,property_id,room_type_id,room_number) VALUES($1::uuid,$2,$3,$1::text)",
                [roomId, propertyId, line.roomTypeId],
              );
              await client.query(
                ...
                [
                  ...
                  JSON.stringify({
                    inventoryReservation: {
                      ...
                      owner: "pms",
                      receiptId: receipts.find((row) => row.room_type_id === line.roomTypeId)!
                        .receipt_id,
                    },
                  }),
                  input.occurredAt,
        ...
        if (source === "guest")
          await client.query(
            "UPDATE booking.booking_change_requests SET status='declined' WHERE id=$1",
            [revision],
          );
        ...
        VALUES($1,$2,$3,$4,'test','edit_dates',$5::jsonb,'{}',$6,$6::timestamptz+interval '10 minutes')`,
            [
              ...
              booking.guestBookingId,
              context.actorUserId,
              JSON.stringify({ checkIn: updated.checkIn, checkOut: updated.checkOut }),
              input.occurredAt,
            ],
          );
        await assign();
        expect(
          (
            await client.query(
              "SELECT lifecycle_state FROM pms.inventory_reservation_statuses WHERE receipt_id=ANY($1::uuid[])",
              [(receipt as PmsInventoryReservationBundle).receipts.map((item) => item.receiptId)],
            )
        ...
        if (source === "host") {
          ...
          expect(
            await wrapped.bundleAvailabilityCredits!({
              ...creditRequest,
              reservation: { ...held, receipts: held.receipts.slice(0, 1) },
            }),
          ).toBeNull();
          const nextAt = new Date(input.occurredAt.getTime() + 1000);
          const nextPreview = await bookingOwner.previewDates(
            client,
            wrapped,
            property,
            updated,
          ...
          VALUES($1,$2,$3,$4,'test','edit_dates',$5::jsonb,'{}',$6,$6::timestamptz+interval '10 minutes')`,
            [
              ...
              booking.guestBookingId,
              context.actorUserId,
              JSON.stringify({ checkIn: input.checkIn, checkOut: input.checkOut }),
              nextAt,
            ],
          );
          const nextOffer = nextPreview.pricingSnapshot!["selectedOffer"] as Record<
            string,
            unknown
          >;
          await client.query("SAVEPOINT failed_amendment");
          await preparePmsHostDateAmendment(client, {
            propertyId,
          ...
          await client.query(
            "UPDATE distribution.public_room_offer_snapshots SET available_rooms=0 WHERE property_id=$1 AND room_type_id=$2",
            [propertyId, rooms[1]],
          );
          await expect(
            bookingOwner.reserveDates(wrapped, nextOffer, {
              transaction: client,
              propertyId,
              quoteSessionId: `host-edit:${nextId}`,
              roomTypeId: rooms[0]!,
              publicOfferKey: rooms[0]!,
              checkIn: input.checkIn,
              checkOut: input.checkOut,
          ...
          expect(
            (
              await client.query(
                "SELECT assignment_status,check_in::text FROM pms.operational_booking_assignments WHERE guest_booking_id=$1 ORDER BY position",
                [booking.guestBookingId],
              )
            ).rows,
          ).toEqual(
          ...
          const amendment = await preparePmsHostDateAmendment(client, {
            propertyId,
            bookingId: booking.guestBookingId,
            previewId: nextId,
            receipt: held,
          ...
          const nextReceipt = await bookingOwner.reserveDates(wrapped, nextOffer, {
            transaction: client,
            propertyId,
            quoteSessionId: `host-edit:${nextId}`,
            roomTypeId: rooms[0]!,
          ...
          await bookingOwner.applyDates(client, {
            booking: updated,
            changeRequest: { id: nextId, hostEdit: true },
            preview: nextPreview,
            selectedOffer: nextOffer,
            inventoryReservation: { ...nextReceipt! },
            context: { ...context, occurredAt: nextAt },
          });
          await completePmsHostDateAmendment(client, {
            propertyId,
            bookingId: booking.guestBookingId,
            previewId: nextId,
            previous: amendment,
          ...
          expect(
            (
              await client.query(
                "SELECT count(*) FROM pms.inventory_reservation_successors WHERE property_id=$1",
                [propertyId],
              )
            ).rows[0].count,
          ).toBe("2");
          ...
          const nestedPool = {
            connect: async () => ({
              query: (sql: string, values?: unknown[]) =>
                client.query(
                  sql === "BEGIN"
                    ? "SAVEPOINT owner_action"
                    : sql === "COMMIT"
                      ? "RELEASE SAVEPOINT owner_action"
                      : sql === "ROLLBACK"
                        ? "ROLLBACK TO SAVEPOINT owner_action"
                        : sql,
                  values,
                ),
              release() {},
          ...
          const hostPreview = await actions.preview(scope, {
            action: "edit_dates",
            reason: "Synthetic mixed host test",
            checkIn: "2027-02-02",
            checkOut: input.checkOut,
          });
          expect(hostPreview.impact.cancellationPolicy).toMatchObject({
            type: "mixed_room",
            lines: [
          ...
          expect(await actions.apply(scope, hostPreview.previewId, randomUUID())).toMatchObject({
          ...
          const finalSnapshot = final.bookingMetadata as {
            policySnapshot: { lines: { totals: { totalAmount: string } }[] };
            selectedOffer: {
              publicPolicy: { lines: { totals: { totalAmount: string } }[] };
              roomLines: { totals: { totalAmount: string } }[];
            };
          };
          const amounts = finalSnapshot.selectedOffer.roomLines.map(
            (line) => line.totals.totalAmount,
          );
          expect(
            finalSnapshot.selectedOffer.publicPolicy.lines.map((line) => line.totals.totalAmount),
          ).toEqual(amounts);
          expect(finalSnapshot.policySnapshot.lines.map((line) => line.totals.totalAmount)).toEqual(
            amounts,
          );
        }
      } finally {
  ...
  it("prefers canonical flexible price and meals without changing exact-offer selection", async () => {
    ...
    try {
      ...
      await client.query(
        ...
        SET rate_summary='{"refundable":true}' WHERE property_id=$1 AND room_type_id=$2`,
        [propertyId, rooms[0]],
      );
      await client.query(
        `INSERT INTO distribution.public_room_offer_snapshots
      ...
      expect(
        await loadTargetCheckoutOffer(client, { ...request, exactPublicOfferKey: rooms[0]! }),
      ).toMatchObject({ publicOfferKey: rooms[0], roomTotal: "200.00" });
      expect(
        await loadTargetCheckoutOffer(client, {
          ...request,
          availabilityCredit: { checkIn: input.checkIn, checkOut: input.checkOut, roomCount: 1 },
        }),
      ).toMatchObject({ publicOfferKey: rooms[0], roomTotal: "200.00" });
      expect(
        await loadTargetCheckoutOffer(client, { ...request, rateType: ratePlanId }),
      ).toMatchObject({ publicOfferKey: key });
      await client.query(
        `UPDATE distribution.public_room_offer_snapshots SET sellable_publicly=false
        WHERE property_id=$1 AND public_offer_key=$2`,
        [propertyId, key],
      );
      expect(await loadTargetCheckoutOffer(client, request)).toMatchObject({
        publicOfferKey: rooms[0],
  ...
  it("requires explicit checkout activation and quotes the complete selection through the adapter", async () => {
    ...
    try {
      ...
      await expect(disabled.quoteBooking(propertyId, request)).rejects.toMatchObject({
        statusCode: 400,
      });
      expect(await enabled.quoteBooking(propertyId, request)).toMatchObject({
        roomSelection: selection,
        roomLines: [
          expect.objectContaining({ roomCount: 2 }),
          expect.objectContaining({ roomCount: 1 }),
        ],
        numberOfRooms: 3,
        totalAmount: 610.25,
        addonTotal: 10.25,
      ...
      await expect(
        enabled.quoteBooking(propertyId, { ...request, adults: 6 }),
  ...
  it("edits mixed to single and back through prepare/save with atomic failure recovery", async () => {
    ...
    expect(
      (
        await pool.query(
          "SELECT current_uses FROM booking.promo_definitions WHERE property_id=$1 AND code='EDIT50'",
          [propertyId],
        )
      ).rows,
    ).toEqual([{ current_uses: 1 }]);
