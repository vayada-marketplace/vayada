...
async function preparePersistence(
  ...
  for (const key of ["receiptId", "attemptId", "jobAttemptId", "propertyId", "connectionId"] as const)
  ...
  return async function persist() {
    ...
    try {
      ...
      if (!attempt.rowCount) throw new Error("Channex availability receipt correlation unavailable");
      await client.query(
