...
export async function recordChannexAlert(
  ...
  const priorIncident = (
    await client.query<{ jobId: string | null }>(
      `SELECT recovery_jobs[1]::text AS "jobId" FROM pms.channel_operational_alerts
       WHERE connection_id=$1::uuid AND binding_generation=$2::uuid AND problem_key=$3 AND resolved_at IS NULL`,
      [binding.id, binding.generation, problemKey],
    )
  ).rows[0];
  if (priorIncident?.jobId) await resolveVerifiedChannexAlert(client, priorIncident.jobId);
...
export async function resolveVerifiedChannexAlert(
  ...
  await client.query(
    `UPDATE pms.channel_operational_alerts alert SET resolved_at=now()
     WHERE alert.resolved_at IS NULL AND $1::uuid=ANY(alert.recovery_jobs)
       ...
       AND EXISTS(SELECT 1 FROM pms.channel_connections connection
         WHERE connection.id=alert.connection_id AND connection.binding_generation=alert.binding_generation)
       AND NOT EXISTS(SELECT 1 FROM unnest(alert.recovery_jobs) AS linked(job_id)
...
export async function listChannexAlerts(
  ...
  const result = await client.query<ChannexAlert>(
    ...
    alert.acknowledged_at AS "acknowledgedAt",COALESCE(alert.resolved_at,recovery_resolution.resolved_at) AS "resolvedAt",alert.recovery_round AS "recoveryRound",
    ($2::boolean AND alert.event_type='non_acked_booking' AND alert.resolved_at IS NULL AND recovery_resolution.resolved_at IS NULL AND EXISTS(
    ...
    LEFT JOIN LATERAL (
      SELECT max(job.finished_at) AS resolved_at
      FROM unnest(alert.recovery_jobs) AS linked(job_id)
      JOIN platform.jobs job ON job.id=linked.job_id
      WHERE alert.resolved_at IS NULL AND cardinality(alert.recovery_jobs)>0
        AND alert.last_occurred_at<=alert.recovery_started_at
        AND NOT EXISTS(SELECT 1 FROM unnest(alert.recovery_jobs) AS candidate(job_id)
          LEFT JOIN platform.jobs pending ON pending.id=candidate.job_id
          WHERE pending.id IS NULL OR pending.status<>'succeeded'
            OR pending.job_metadata->>'alertRecoveryVerified' IS DISTINCT FROM 'true')
    ) recovery_resolution ON true
    WHERE alert.property_id=$1::uuid ORDER BY COALESCE(alert.resolved_at,recovery_resolution.resolved_at) NULLS FIRST,alert.last_occurred_at DESC LIMIT 100`,
    [propertyId, staging],
  );
  return result.rows;
}
