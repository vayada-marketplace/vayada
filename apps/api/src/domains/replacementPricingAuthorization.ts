...
export async function lockReplacementPricingAuthorization(
  ...
  if (
    ...
    context.selectedOrganization.organizationId.toLowerCase() !==
    ...
    !["read", "manage", "preview"].includes(operation)
  )
    return false;
  const propertyId = scope.propertyId.toLowerCase();
  const required =
    operation === "preview"
      ? (["pms.rooms_rates.read", "pms.operations.read"] as const)
  ...
  if (
    !required.every((permission) => hasPermission(context, permission)) ||
  ...
  const result = await client.query(
    `SELECT m.role_key,m.property_access_mode,m.permission_overrides
    ...
    FOR SHARE OF m,u,p`,
    [context.membership.membershipId, scope.organizationId, scope.actorUserId, propertyId],
  );
  const membership = result.rows[0];
  if (
    !membership ||
    membership.role_key !== context.membership.roleKey ||
    !["all", "assigned"].includes(membership.property_access_mode) ||
    (membership.role_key === "external_owner" && membership.property_access_mode !== "assigned")
  )
    return false;
  const links = (
    await client.query(
      `SELECT product,resource_type FROM identity.organization_resource_links
  ...
  if (!links.some((l) => l.product === "pms") || !links.some((l) => l.product === "hotel_catalog"))
    return false;
  if (membership.property_access_mode === "assigned") {
    const assignment = await client.query(
      `SELECT property_id FROM identity.membership_property_assignments
      WHERE membership_id=$1 AND property_id=$2 FOR SHARE`,
      [context.membership.membershipId, propertyId],
    );
    if (!assignment.rowCount) return false;
  }
  const rolePermissions = (
    await client.query(
      `SELECT permission_key FROM identity.role_permission_grants
    WHERE organization_kind='hotel_group' AND role_key=$1 FOR SHARE`,
      [membership.role_key],
    )
  ).rows.map((r) => r.permission_key as string);
...
export async function lockCurrentPmsPricingEntitlement(
  ...
  const entitlements = (
    await client.query(
      `SELECT * FROM identity.product_entitlements
