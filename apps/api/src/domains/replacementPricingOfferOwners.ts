...
export type ReplacementPricingOfferOwners =
  | { kind: "verified"; terms: readonly ReplacementOfferTerms[]; finance: Extract<FinanceReplacementPricingReadiness, { kind: "ready" }> }
  | { kind: "unavailable"; reason: "invalid" | "denied" | "room_unavailable" | "terms_stale" | "finance_unavailable";
 ...
 * holds PMS/Booking/Finance locks until its end. Verifies these owners only: not a
 * complete publication/source guard, charge confirmation or currency-change approval. */
export async function lockReplacementPricingOfferOwners(client: PoolClient, context: RequestContext | null,
  scope: PricingStorageScope, proposed: unknown): Promise<ReplacementPricingOfferOwners> {
  ...
  return finance.kind === "ready" ? { kind: "verified", terms, finance } :
    { kind: "unavailable", reason: "finance_unavailable", financeReason: finance.reason };
}
