...
describe.skipIf(!databaseUrl)("non-earning validation probe storage", () => {
  ...
  beforeEach(async () => {
    await fixture.pool().query(`
      DROP SCHEMA IF EXISTS booking, platform CASCADE;
      CREATE SCHEMA booking;
      CREATE SCHEMA platform;
      CREATE FUNCTION platform.prevent_append_only_mutation() RETURNS trigger
      ...
      INSERT INTO identity.organizations VALUES ('${id(4)}'), ('${id(7)}');
      INSERT INTO booking.affiliate_destination_versions VALUES ('${id(30)}','${id(3)}','${id(4)}');
    `);
  ...
  it("enforces destination scope, bounded expiry, purpose and environment", async () => {
    ...
    await expect(
      fixture
        .pool()
        .query(
          `INSERT INTO booking.affiliate_validation_probe_revocations(probe_id,actor_id,organization_id,request_id) VALUES($1,$2,$3,'wrong-scope')`,
          [id(50), id(1), id(7)],
        ),
    ).rejects.toMatchObject({ code: "23503" });
    expect(
