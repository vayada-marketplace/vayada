...
import {
  ...
  parseFinanceFolioExportJobPayload,
...
// prettier-ignore
export async function runFinanceFolioExportJobs(pool: pg.Pool, read: Pick<FinanceFolioReadRepository, "exportReady">, writer: FinanceFolioExportArtifactWriter, options: Options = {}): Promise<FinanceFolioExportCounters> {
...
// prettier-ignore
async function runOne(pool: pg.Pool, read: Pick<FinanceFolioReadRepository, "exportReady">, writer: FinanceFolioExportArtifactWriter, options: Options): Promise<keyof FinanceFolioExportCounters | null> {
  const client = await pool.connect(), clock = options.clock ?? (() => new Date()), now = clock(), workerId = options.workerId ?? `finance-folio-export:${process.pid}`;
  try {
    await client.query("BEGIN");
    await client.query("SET LOCAL lock_timeout='3s'; SET LOCAL statement_timeout='45s'");
    const job = (await client.query<Job>(`SELECT id::text,property_id::text AS "propertyId",resource_type AS "resourceType",resource_id AS "resourceId",correlation_id AS "correlationId",idempotency_key_hash AS "idempotencyKeyHash",attempts_count::int AS "attemptsCount",max_attempts::int AS "maxAttempts",status,payload,job_metadata->>'organizationId' AS "organizationId",job_metadata->>'actorUserId' AS "actorUserId",job_metadata->'responseEnvelope'->>'currency' AS currency,job_metadata->>'acceptedAt' AS "acceptedAt",job_metadata->>'snapshotAt' AS "snapshotAt",job_metadata->>'expiresAt' AS "expiresAt",job_metadata->>'payloadFingerprint' AS "payloadFingerprint",job_metadata->>'manifestDigest' AS "manifestDigest",job_metadata->>'formatVersion' AS "formatVersion",job_metadata->>'requestId' AS "requestId",job_metadata->>'causationId' AS "causationId"
      FROM platform.jobs WHERE queue_name=$1 AND job_type=$2 AND tenant_scope='property' AND property_id IS NOT NULL AND attempts_count<=max_attempts AND ((status='pending' AND run_after<=$3::timestamptz AND attempts_count<max_attempts) OR (status='running' AND locked_at<$3::timestamptz-interval '5 minutes')) ORDER BY priority DESC,run_after,created_at FOR UPDATE SKIP LOCKED LIMIT 1`, [FINANCE_FOLIO_EXPORT_QUEUE, FINANCE_FOLIO_EXPORT_JOB, now.toISOString()])).rows[0];
    if (!job) { await client.query("COMMIT"); return null; }
    if (job.status === "running") {
      const stale = (await client.query<{id:string}>("UPDATE platform.job_attempts SET status='timed_out',finished_at=$3,error_type='worker_timeout',error_message='Finance folio export worker lease expired.' WHERE job_id=$1::uuid AND attempt_number=$2 AND status='running' RETURNING id::text", [job.id, job.attemptsCount, now.toISOString()])).rows[0];
      if (!stale) throw new Error("Finance folio export running attempt is missing");
    ...
    let artifact: FinanceFolioCsvArtifact;
    try {
      const payload = parseFinanceFolioExportJobPayload(job.payload, { organizationId: job.organizationId, propertyId: job.propertyId, currency: job.currency, payloadFingerprint: job.payloadFingerprint, acceptedAt: job.acceptedAt, snapshotAt: job.snapshotAt, expiresAt: job.expiresAt, now });
      if (!uuid(job.actorUserId) || job.formatVersion !== FINANCE_FOLIO_CSV_VERSION || hash(JSON.stringify(payload.snapshot.manifest)) !== job.manifestDigest) throw new TypeError("Finance folio export metadata is invalid");
      const rendered = await read.exportReady(job.propertyId, job.currency, payload.snapshot);
      if (!rendered || !validArtifact(rendered, job.propertyId, job.currency, payload.snapshot.manifest)) throw new FinanceFolioEvidenceError("Folio export evidence is unavailable");
      artifact = rendered;
    } catch (error) {
      const failure = error instanceof ExecutionFailure ? error : error instanceof TypeError || error instanceof FinanceFolioEvidenceError ? new ExecutionFailure("invalid_export_evidence", false) : new ExecutionFailure("export_read_failed", true);
    ...
    try { stored=await writer.write({exportId:job.id,body:artifact.body,contentType:artifact.contentType,expiresAt:job.expiresAt});if(!validStored(stored,writer.bucketName,job.id,artifact.body))throw new Error("Invalid private artifact receipt"); }
    ...
    await audit(client, job, attempt, "succeeded", completedAt, undefined, { rowCount: artifact.rowCount, manifestCount: artifact.auditEvidence.length, checksumSha256: stored.checksumSha256 });
...
// prettier-ignore
function validArtifact(artifact:FinanceFolioCsvArtifact,propertyId:string,currency:string,manifest:readonly {folioId:string;revision:number;sourceDigest:string}[]){return artifact.propertyId===propertyId&&artifact.currency===currency&&artifact.formatVersion===FINANCE_FOLIO_CSV_VERSION&&artifact.contentType===FINANCE_FOLIO_CSV_CONTENT_TYPE&&artifact.filename===`pms-financials-folios-${propertyId}.csv`&&Number.isSafeInteger(artifact.rowCount)&&artifact.rowCount>=0&&artifact.body.length>0&&JSON.stringify(artifact.auditEvidence)===JSON.stringify(manifest.map(({folioId,revision,sourceDigest})=>({folioId,revision,sourceDigest})));}
// prettier-ignore
function validStored(stored:FinanceFolioExportArtifact,bucketName:string,exportId:string,body:string){return stored.bucketName===bucketName&&stored.storageKey===storageKey(exportId)&&stored.sizeBytes===Buffer.byteLength(body,"utf8")&&stored.checksumSha256===hash(body);}
const storageKey = (exportId: string) =>
