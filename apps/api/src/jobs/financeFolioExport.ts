...
// prettier-ignore
async function deadLetter(client:pg.PoolClient,job:Job,attemptId:string,attempt:number,code:string,replayEligible:boolean,now:Date){await client.query(`INSERT INTO platform.dead_letter_events(source_kind,job_id,job_attempt_id,tenant_scope,property_id,resource_product,resource_type,resource_id,correlation_id,idempotency_key_hash,reason_code,failure_summary,failure_payload,created_at) VALUES('job',$1::uuid,$2::uuid,'property',$3::uuid,'finance',$4,$5,$6,$7,$8,$9,jsonb_build_object('attemptNumber',$10::int,'affectedOrganizationId',$11::text,'lastAttemptAt',$12::text,'ownerPackage','backend-events','replayEligible',$13::boolean),$12::timestamptz)`,[job.id,attemptId,job.propertyId,job.resourceType,job.resourceId,job.correlationId,job.idempotencyKeyHash,code,`Finance ${jobTab(job)} export failed (${code}).`,attempt,job.organizationId,now.toISOString(),replayEligible]);}
// prettier-ignore
