...
| V6  | **Notifications, newsletter, invite codes** | `/notifications*` (2), `/newsletter/*` (2), `/admin/invite-codes*` + `/api/invite-codes/*` (5)              | MW, VA, BA | Small surfaces. `marketplace_notifications` stays product-owned (not platform job delivery state), but `GET /notifications` and `POST /notifications/{id}/read` currently have **no frontend consumer** — confirm retire vs migrate when V6 is ticketed. Invite-code admin CRUD (VA) plus public lookup/redeem consumed by booking-admin's setup flow (`apps/booking-admin/app/setup/page.tsx`); the V6 cutover must include booking-admin or hotel onboarding breaks. |
