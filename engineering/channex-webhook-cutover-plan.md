...
2. Set the failed provider's target route to `observe_only` with durable receipt
   writes. Do not proxy to legacy before the verified cutoff or use a
   receipt-less 2xx ack.
3. Stop manual repair and pause target promotion/retry workers for that provider.
   Drain in-flight target jobs, record the cutoff, and verify no target mutation
   remains in flight before restoring legacy ownership.
4. Repoint the provider dashboard endpoint to the exported legacy URL.
5. Set the matching legacy route to `mutating`. Keep target promotion and retry
   paused while legacy owns the stream.
6. Keep schedulers frozen until the domain owner confirms the provider/domain
   has no target job still running; then re-enable only the jobs required for
   the rolled-back provider.
