...
| `POST /webhooks/stripe/connect` Connect `account.updated`          | Marks hotel or affiliate onboarding state                                                                                             | Not implemented; endpoint switch blocked until distinct signing secret, route, and signed delivery are verified | Finance/identity entitlement read model                                                                       | `ack_only_with_receipt` or `proxy_to_target` for 72 hours | Follow the [drain-before-restore sequence](#roll-back-during-endpoint-switch); reconcile account state from Stripe                          |
