...
8. **Affiliate public Booking Web proxies** (`PMS_PUBLIC_API_URL` affiliate
   calls).
   Assign ownership before implementation: either marketplace/affiliate target
   services own registration and Stripe Connect, or the Booking Web routes are
   retired if no product surface still needs them.
...
| Booking Web affiliate routes                    | `PMS_PUBLIC_API_URL` affiliate calls                                                                | accepted affiliate owner config, likely `TARGET_DATABASE_URL` plus `AFFILIATE_PUBLIC_SOURCE=target`, or route retirement flag | Public affiliate routes either mount against target ownership or return the accepted retired-contract response with no PMS proxy.                                                        |
...
8. **Decide and implement Booking Web affiliate route ownership**
   - Either port check-email/register/Stripe Connect link to target
     marketplace/affiliate ownership or retire the public routes and update
     consumers.
   - Validation: accepted owner has route contract tests and no PMS proxy.
