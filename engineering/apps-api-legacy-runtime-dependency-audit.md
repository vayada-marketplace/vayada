...
| Booking Web affiliate routes                    | `PMS_PUBLIC_API_URL` affiliate calls                                                                | `TARGET_DATABASE_URL` plus `AFFILIATE_PUBLIC_SOURCE=target`; route retirement flag only if the Booking Web referral modal is removed | Public affiliate routes mount against marketplace/affiliate plus finance target ownership, or return the accepted retired-contract response only after the public UI is removed, with no PMS proxy either way. |
...
8. **Implement Booking Web affiliate route ownership**
   - Port check-email/register to target marketplace/affiliate ownership and
     ...
     `AFFILIATE_PUBLIC_SOURCE=target`; keep route retirement only for a later
     product decision that removes the Booking Web referral modal.
