...
Status: proposed and reconciled with `main` on September 16, 2026. No product or
privacy/legal approval is recorded. Delivery implementation remains blocked.
...
| ----------------------------------- | ------------------ | -------------------------------------------------------------------------- |
| New application or invitation email | `immediate`, `off` | Unresolved; use `off` until the privacy/legal default decision is recorded |
| All Marketplace email               | `on`, `off`        | Unresolved; use `off` until the same decision is recorded                  |
...
The proposal treats actionable collaboration updates as product-service
...
discovery communications and must remain non-promotional. Privacy/legal review
must confirm this classification for Vayada's operating jurisdictions before a
default or rollout is approved.
...
No approval was found in VAY-1029 or PR #1321 as of September 16, 2026. Record
these decisions before implementation children leave Backlog.

### Product decisions

1. Approve the 48-hour counterparty action outcome, organization-level
   treatment assignment, and one-count-per-collaboration analysis.
2. Approve email as the only MVP channel and new application/invitation as the
   only trigger, with the audience and content boundaries above.
3. Approve the preference shape: `immediate` or `off` for this topic, an all-
   Marketplace-email switch, direct category opt-out, and no reminder or digest
   cadence in MVP.
4. Approve the staged holdout rollout and keeping Instagram, in-app, push,
   reminders, and digests outside MVP.

### Privacy/legal decision

Classify the action-required email for each operating jurisdiction and choose
the initial state separately for existing active members and newly activated
members. Do not encode a default while that decision is absent: the safe
fallback is `off`. If privacy/legal confirms the message is a non-promotional
service communication, product may approve `immediate` by default with the
category and global opt-outs above. If it is marketing or the classification
remains uncertain, require explicit opt-in. Legacy newsletter rows never supply
that permission.

### Provider readiness, not product preference

Resend is the recommended implementation direction because it is already used
by the target API. Before any real send, verify the actual Vayada account's
sender/domain, SPF/DKIM/DMARC, quota and rate limit, API/webhook secret ownership,
bounce/complaint/suppression events, and operational owner. These checks cannot
substitute for the product and privacy/legal decisions above.

The concrete next decision is for the product owner to approve or reject the
email-only application/invitation MVP. If approved, the privacy/legal owner must
then record the service-versus-marketing classification and default-on versus
explicit-opt-in rule before contract or implementation work starts.
