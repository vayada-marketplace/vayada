...
The 77 `active` table entries in `packages/backend-migration/source-inventory.tsv` retain their existing target owner, fixture case, parity category, and retention disposition. The six additional PMS tables have historical-only dispositions in `packages/backend-migration/raw-source-dispositions.tsv`. The two files together account for all 83 observed table names. The extractor includes those six **only** when the reviewed manifest supplies `historicalInventorySha256` matching the exact sidecar file; the run ID changes with that hash. Existing VAY-1350 extraction runs remain unchanged. The isolated run must still verify their actual row counts/checksums before rehearsal. Neither `vayada_pms_staging` nor any `target_*` database is a legacy source.
...
Archived inbox prototype definitions/templates and public automation records are historical evidence only. The existing encrypted RDS source snapshot retains them; the opt-in extractor stages their raw rows under the approved rollback retention. Do not load them as runnable schedules, templates, outbound queue entries, or sends. Compare public/archive send IDs and lineage before deduplication or parity counting. No provider call or message replay is part of normalization.
