...
const normalized = `lower(btrim(email, U&'\\0009\\000a\\000b\\000c\\000d\\0020\\00a0\\1680\\2000\\2001\\2002\\2003\\2004\\2005\\2006\\2007\\2008\\2009\\200a\\2028\\2029\\202f\\205f\\3000\\feff'))`;
...
export function planLegacyOwnerEmailIndex(emailSha256: readonly string[]) {
  if (
    !Array.isArray(emailSha256) ||
    emailSha256.length !== 8 ||
    [...emailSha256].some((hash) => typeof hash !== "string" || !/^[0-9a-f]{64}$/.test(hash)) ||
    new Set(emailSha256).size !== 8
  )
    throw new Error("INVALID_OWNER_EMAIL_GUARD_SCOPE");
  const hashes = [...emailSha256].sort();
