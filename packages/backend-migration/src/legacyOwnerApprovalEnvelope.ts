 ...
 * `freshness` is informational so an authenticated exact-success replay can be
 * classified before expiry blocks a new execution. Callers must require
 * `current` before any new mutation.
...
export function verifyLegacyOwnerApprovalEnvelope(input: {
  ...
  envelope: LegacyOwnerApprovalEnvelope;
  freshness: "current" | "not_yet_valid" | "expired";
} {
  ...
  if (!Number.isFinite(input.now.getTime())) return fail("invalid_clock");
  const now = input.now.toISOString();
  const freshness =
    envelope.issuedAt > now ? "not_yet_valid" : envelope.expiresAt <= now ? "expired" : "current";
  return { outcome: "signature_matches_requires_registry", envelope, freshness };
}
