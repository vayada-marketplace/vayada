...
it.skipIf(!url)("seeds account roles once without changing legacy access", async () => {
  ...
  try {
    ...
    await client.query(
      await readFile(
        new URL("../migrations/0196_organization_role_definitions.sql", import.meta.url),
        "utf8",
      ),
    );
    ...
    await client.query(
      await readFile(new URL("../migrations/0197_team_role_presets.sql", import.meta.url), "utf8"),
    );
    await client.query(
