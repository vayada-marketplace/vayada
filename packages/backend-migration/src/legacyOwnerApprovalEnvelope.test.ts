...
describe("legacy owner evidence envelope, synthetic signing keys only", () => {
  ...
  it.each([
    ...
    ["expiresAt", "2026-09-14T01:00:00.000Z", "invalid_time_window"],
  ...
  it.each([
    ["issuedAt", "2026-09-14T01:31:00.000Z", "not_yet_valid"],
    ["expiresAt", "2026-09-14T01:30:00.000Z", "expired"],
    ["expiresAt", "2026-09-14T01:29:59.999Z", "expired"],
  ] as const)(
    "authenticates signed %s outside its execution window as %s",
    (field, value, freshness) => {
      ...
      ).toMatchObject({ outcome: "signature_matches_requires_registry", freshness });
    },
  );
