...
CREATE TABLE pms.channex_offer_create_attempts (
  ...
  state TEXT NOT NULL DEFAULT 'unresolved' CHECK (state IN ('unresolved','identified','released')),
  external_rate_plan_id TEXT CHECK (external_rate_plan_id <> '' AND external_rate_plan_id = btrim(external_rate_plan_id)),
  created_at TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp(),
  CHECK ((state IN ('unresolved','released') AND external_rate_plan_id IS NULL) OR
...
CREATE UNIQUE INDEX channex_offer_one_active_create_intent
  ON pms.channex_offer_create_attempts(intent_id) WHERE state<>'released';
...
BEGIN
  ...
  ELSE
    ...
    IF (NEW.id,NEW.intent_id,NEW.target_id,NEW.version,NEW.binding_generation,
      ...
      OR OLD.state <> 'unresolved'
      OR NOT ((NEW.state='identified' AND NEW.external_rate_plan_id IS NOT NULL) OR
              (NEW.state='released' AND NEW.external_rate_plan_id IS NULL)) THEN
      RAISE EXCEPTION 'Channex creation identity and terminal state retained' USING ERRCODE='23514';
    END IF;
    IF NEW.state='identified' THEN
