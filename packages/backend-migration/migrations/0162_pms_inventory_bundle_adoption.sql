-- VAY-910: adopt a complete receipt bundle; retain existing single-receipt and room-move guards.
    ...
    IF NOT EXISTS (
      WITH tokens AS (
        ...
        WHERE receipt.property_id=assignment_row.property_id
          AND receipt.quote_session_id=booking_row.quote_session_id::text
      ...
      HAVING count(*) BETWEEN 1 AND 99
        ...
        AND count(*)=(SELECT count(*) FROM pms.inventory_reservation_receipts r
          WHERE r.property_id=assignment_row.property_id AND r.quote_session_id=booking_row.quote_session_id::text)
