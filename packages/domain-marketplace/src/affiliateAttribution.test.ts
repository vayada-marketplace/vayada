...
import * as marketplacePublicApi from "./index.js";
...
it("excludes unrelated bookings/properties, test and ineligible clicks without hiding the older eligible click", () => {
  const candidates = [
    ...
    ...[
      ...
      click({
        clickId: `excluded-${i}`,
        historyPosition: String(i + 2),
        clickedAt: "2026-09-10T11:00:00Z",
        ...override,
      }),
    ),
  ];
...
it("excludes clicks committed after the original booking cutoff even when their source time is earlier", () => {
  expect(
    select(
      input([
        click(),
  ...
  ).toMatchObject({ status: "attributed", clickId: "click-1" });
  expect(
    select(
      input([
        click(),
        click({
          clickId: "malformed-after-cutoff",
          historyPosition: "11",
          clickedAt: "not-a-time",
          agreementId: "",
        }),
      ]),
    ),
...
it("keeps the trusted selector outside the package public API", () => {
  expect("selectLastEligibleAffiliateClick" in marketplacePublicApi).toBe(false);
});
it.each([
...
it("requires each trusted history position to identify one occurrence", () => {
  expect(
    select(
      input([
        click(),
        click({
          clickId: "different-click",
          historyPosition: "1",
          clickedAt: "2026-09-10T11:00:00Z",
        }),
      ]),
    ),
  ).toEqual({ status: "needs_review", reason: "conflicting_history_position" });
});
it("does not use array order or lexical IDs to break a tied latest click", () => {
  const candidates = [
    click(),
    click({ clickId: "other", creatorProfileId: "other", historyPosition: "2" }),
  ];
  expect(select(input(candidates))).toEqual({
    status: "needs_review",
...
it("rejects impossible dates, implicit timezones and missing references instead of crediting", () => {
  ...
  for (const clickHistoryCutoff of ["", "-1", "01", "1.5", "bad"])
    expect(select({ ...input([click()]), clickHistoryCutoff })).toMatchObject({
      status: "needs_review",
    });
  for (const historyPosition of ["", "0", "-1", "01", "1.5", "bad"])
    expect(select(input([click({ historyPosition })]))).toMatchObject({
      status: "needs_review",
    });
});
