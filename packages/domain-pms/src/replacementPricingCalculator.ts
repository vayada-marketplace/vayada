...
export type RoomStayPricingRequest = Readonly<{
  propertyId: string;
  roomTypeId: string;
  offerId: string;
...
export type RoomNightProjectionRequest = Omit<RoomStayPricingRequest, "checkIn" | "checkOut"> &
  Readonly<{ date: string }>;
export type PricedRoomNight = Readonly<{
  date: string;
  roomMinor: string;
  mealMinor: string;
  totalMinor: string;
  sources: readonly Readonly<{
    offerId: string;
    kind: "date" | "season" | "month" | "base" | "weekday" | "linked";
  }>[];
}>;
type ProjectedRoomNight = PricedRoomNight &
  Readonly<{ restrictions: PricingRestrictions; restrictionOfferId: string }>;
export type RoomStayPricingResult =
  | Readonly<{
      kind: "priced";
      version: "pricing.v2";
      propertyId: string;
...
export type RoomNightProjectionResult =
  | Extract<RoomStayPricingResult, { kind: "unavailable" }>
  | Readonly<
      Omit<
        Extract<RoomStayPricingResult, { kind: "priced" }>,
        "kind" | "nights" | "roomMinor" | "mealMinor" | "totalMinor"
      > & { kind: "projected"; night: ProjectedRoomNight }
    >;
type EvaluatedRoomStayResult =
  | Extract<RoomStayPricingResult, { kind: "unavailable" }>
  | Readonly<
      Omit<Extract<RoomStayPricingResult, { kind: "priced" }>, "nights"> & {
        nights: readonly ProjectedRoomNight[];
      }
    >;
type PricingPeriod =
  | { kind: "stay"; checkIn: string; checkOut: string }
...
export function calculateReplacementRoomStay(
  configuration: unknown,
  request: RoomStayPricingRequest,
): RoomStayPricingResult {
  ...
  return {
    ...result,
    nights: result.nights.map(
      ({ restrictions: _rules, restrictionOfferId: _owner, ...night }) => night,
    ),
  };
}
/** One calendar night's tariff and restrictions, not a stay quote or an availability decision. */
export function projectReplacementRoomNight(
  configuration: unknown,
...
function evaluateRoomPricing(
  ...
  if (
    !request ||
    !pricingDate(firstDate) ||
    (period.kind === "stay" && (!pricingDate(period.checkOut) || period.checkOut <= firstDate)) ||
    !pricingInteger(request.expectedRevision, 1) ||
    request.propertyId !== config.propertyId ||
    request.roomTypeId !== config.roomTypeId ||
    !pricingObject(request.expectedTermsRevisions)
  )
  ...
  if (!request.guests || !validPricingGuests(request.guests, config.capacity, config.children))
    return { kind: "unavailable", reason: "invalid_guests" };
  ...
  try {
    const chain: PricingOffer[] = [],
      terms: Record<string, string> = Object.create(null);
    ...
    while (
      restrictionOwner.restrictions.kind === "inherit" &&
      restrictionOwner.price.kind === "linked"
    )
      restrictionOwner = offers.get(restrictionOwner.price.parentId)!;
    const restrictions = (date: string): PricingRestrictions => {
      ...
      return (
        policy.dates.find((r) => r.date === date)?.rules ??
        policy.seasons.find((r) => inSeason(date.slice(5), r.from, r.through))?.rules ??
        policy.rules
      );
    };
    const stayLength =
      period.kind === "stay"
        ? (Date.parse(period.checkOut) - Date.parse(firstDate)) / 86400000
        : null;
    const arrival = restrictions(firstDate);
    if (
      period.kind === "stay" &&
      (arrival.closedToArrival ||
        arrival.minArrivalNights > stayLength! ||
        restrictions(period.checkOut).closedToDeparture)
    )
      return fail("restriction");
    const young = request.guests.childAgesAtCheckIn.filter(
      (age) => age < config.children.adultFromAge,
    );
    const adults = request.guests.adults + request.guests.childAgesAtCheckIn.length - young.length;
    const bands = young.map((age) =>
      config.children.bands.findIndex((b) => age >= b.fromAge && age <= b.throughAge),
    );
    const supplement = bound(
      bands.reduce((sum, index) => sum + BigInt(config.children.bands[index].nightlyMinor), 0n),
    );
    const charge = selected.meal.charge;
    const meal = bound(
      charge.kind === "room"
        ? BigInt(charge.amountMinor)
        : BigInt(charge.adultMinor) * BigInt(adults) +
            bands.reduce((sum, index) => sum + BigInt(charge.childBandAmountsMinor[index]), 0n),
    );
    const nights: ProjectedRoomNight[] = [];
    let roomTotal = 0n,
      mealTotal = 0n;
    for (
      let time = Date.parse(firstDate),
        last = period.kind === "stay" ? Date.parse(period.checkOut) - 86400000 : time;
      time <= last;
      time += 86400000
    ) {
      const date = new Date(time).toISOString().slice(0, 10),
        rule = restrictions(date);
      if (
        stayLength !== null &&
        (rule.stopSell || (rule.maxStayNights !== null && stayLength > rule.maxStayNights))
      )
        return fail("restriction");
      let amount = 0n;
      let sources: Array<PricedRoomNight["sources"][number]> = [];
      const lastOverride = chain.findLastIndex((o) =>
        (o.price.kind === "linked" ? o.price.dateOverrides : o.price.calendar.dates).some(
          (r) => r.date === date,
        ),
      );
      ...
      roomTotal = bound(roomTotal + amount);
      mealTotal = bound(mealTotal + meal);
      nights.push({
        date,
        restrictions: rule,
        restrictionOfferId: restrictionOwner.id,
        roomMinor: amount.toString(),
        mealMinor: meal.toString(),
        totalMinor: bound(amount + meal).toString(),
        sources,
      });
    }
    return {
      ...
      guests: structuredClone(request.guests),
      termsRevisions: terms,
      nights,
      roomMinor: roomTotal.toString(),
      mealMinor: mealTotal.toString(),
      totalMinor: bound(roomTotal + mealTotal).toString(),
    };
  } catch (error) {
    if (error instanceof Unavailable) return { kind: "unavailable", reason: error.reason };
    throw error;
  }
}
